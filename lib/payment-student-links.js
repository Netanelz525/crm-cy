...
import { ensurePaymentLinkTables, upsertPaymentRecordLink } from "./payment-links.js";
...
function safeDate(value) {
  ...
  const localDate = clean(value).match(/^(\d{1,2})[\/.](\d{1,2})[\/.](\d{2,4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2}))?)?$/);
  if (localDate) {
    const [, day, month, rawYear, hour = "12", minute = "00", second = "00"] = localDate;
    const year = rawYear.length === 2 ? `20${rawYear}` : rawYear;
    const parsed = new Date(`${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}T${hour.padStart(2, "0")}:${minute}:${second}`);
...
export async function linkTransactionsFromMandates({ studentId = "" } = {}) {
  await ensurePaymentLinkTables();
  const mandates = await sql`
    SELECT * FROM payment_record_links
    WHERE record_type = 'mandate'
      AND (${clean(studentId)} = '' OR student_id = ${clean(studentId)})
  `;
  const saved = [];
  for (const mandate of mandates) {
    const snapshot = mandate.record_snapshot || {};
    const rows = await sql`
      SELECT pr.* FROM payment_records pr
      WHERE pr.record_type = 'transaction'
        AND pr.provider = ${mandate.provider}
        AND pr.connection_id = ${mandate.connection_id}
        AND NOT EXISTS (
          SELECT 1 FROM payment_record_links existing
          WHERE existing.record_type = 'transaction'
            AND existing.provider = pr.provider
            AND existing.connection_id = pr.connection_id
            AND existing.external_record_id = pr.external_record_id
        )
    `;
    for (const row of rows) {
      const transaction = row.payload_json || {};
      const directDebit = clean(transaction.directDebitNumber || transaction.mandateId || transaction.raw?.subscription);
      const mandateId = clean(mandate.external_record_id);
      const directMatch = Boolean(directDebit && mandateId && directDebit === mandateId);
      const identityMatches = [
        digits(row.donor_id) && digits(row.donor_id) === digits(snapshot.donorId),
        email(row.email) && email(row.email) === email(mandate.payer_email || snapshot.email),
        digits(row.phone) && digits(row.phone) === digits(mandate.payer_phone || snapshot.phone)
      ].filter(Boolean).length;
      if (!directMatch && identityMatches < 2) continue;
      const link = await upsertPaymentRecordLink({
        recordType: "transaction", provider: mandate.provider, connectionId: mandate.connection_id,
        externalRecordId: row.external_record_id, studentId: mandate.student_id,
        payerType: mandate.payer_type, payerName: mandate.payer_name || row.customer_name,
        payerEmail: mandate.payer_email || row.email, payerPhone: mandate.payer_phone || row.phone,
        notes: "שויך אוטומטית דרך הוראת קבע", recordSnapshot: transaction,
        linkedByUserId: mandate.linked_by_user_id
      });
      await linkPaymentToStudent({ paymentRecordId: row.id, studentId: mandate.student_id, userId: mandate.linked_by_user_id || "" });
      saved.push(link);
    }
  }
  return saved;
...
export async function getCachedPaymentTransactions({ connections = [], dateFrom, dateTo, refresh = false }) {
  ...
  const cachedRows = await sql`
    ...
    WHERE record_type = 'transaction'
      ...
      AND payload_json->>'createdAt' ~ '^\\d{1,2}[/.]\\d{1,2}[/.]\\d{2,4}'
  `;
  for (const row of cachedRows) {
    const correctedDate = safeDate(row.payload_json?.createdAt);
    const currentDate = row.occurred_at ? new Date(row.occurred_at) : null;
    if (correctedDate && (!currentDate || correctedDate.getTime() !== currentDate.getTime())) {
      await sql`UPDATE payment_records SET occurred_at = ${correctedDate} WHERE id = ${row.id}`;
    }
  }
  const rows = await sql`
  ...
  const metadata = await sql`
    ...
    WHERE connection_id = ANY(${activeConnectionIds})
      AND scanned_date >= ${from}::date AND scanned_date <= ${to}::date
  `;
  const inheritedLinks = await linkTransactionsFromMandates();
  return {
