...
function mapNedarimMandate(connection, row) {
  ...
  const status = hasIssue ? "issues" : "active";
  const statusLabel = hasIssue ? "עם תקלות" : "פעיל";
...
function mapStripeMandate(connection, subscription) {
  ...
  const hasIssue = ["past_due", "unpaid", "paused"].includes(status);
  const issueText = hasIssue
    ? clean(
        latestInvoice?.last_finalization_error?.message
  ...
  return {
    ...
    netAmount: amount,
    feeAmount: 0,
    status: hasIssue ? "issues" : "active",
    statusLabel: hasIssue ? `עם תקלות (${clean(subscription?.status)})` : clean(subscription?.status),
    createdAt,
    createdAtUnix,
    nextChargeDate,
...
async function fetchNedarimMandates(connection, { dateFrom, dateTo }) {
  const pageSize = 1000;
  let lastId = "0";
  const allMandates = [];

  async function requestPage(method, currentLastId) {
    const params = new URLSearchParams({
      Action: "GetKevaJson",
      MosadId: clean(connection.external_id),
      ApiPassword: clean(connection.secret),
      LastId: clean(currentLastId || "0"),
  ...
  let method = "GET";
  try {
    await requestPage("GET", lastId);
  } catch {
    method = "POST";
  }

  for (let page = 0; page < 20; page += 1) {
    const chunk = await requestPage(method, lastId);
  ...
  return allMandates
    ...
    .filter(Boolean)
    .filter((item) => !dateFrom || !dateTo || isDateWithinRange(item.createdAt, dateFrom, dateTo));
}

async function fetchStripeMandates(connection, { dateFrom, dateTo }) {
  const allMandates = [];
  let startingAfter = "";
  ...
  do {
    ...
    chunk.forEach((subscription) => {
      ...
      if (!mapped) return;
      if (dateFrom && dateTo && !isDateWithinRange(mapped.createdAt, dateFrom, dateTo)) return;
...
export async function getPaymentMandatesDashboard({
  ...
  const results = await Promise.allSettled(
    connections.map(async (connection) => {
      if (connection.provider === "stripe") {
        return fetchStripeMandates(connection, { dateFrom, dateTo });
      }
      return fetchNedarimMandates(connection, { dateFrom, dateTo });
    })
  );
  ...
  const enrichedMandates = await enrichTransactionsWithIlsAmounts(mandates, { dateFrom, dateTo });
