...
async function loadXlsxModule() {
  if (!xlsxModulePromise) {
    xlsxModulePromise = import("xlsx").then((mod) => mod?.default?.utils ? mod.default : mod);
  }
  return xlsxModulePromise;
}

async function parseCsvWithXlsx(input) {
  const XLSX = await loadXlsxModule();
  const text = typeof input === "string" ? input.replace(/^\uFEFF/, "") : "";
  const workbook = typeof input === "string"
    ? XLSX.read(text, { type: "string", FS: text.includes("\t") ? "\t" : "," })
    : XLSX.read(Buffer.from(input instanceof Uint8Array ? input : new Uint8Array(input || 0)), { type: "buffer", raw: false, FS: "\t" });
  const sheet = workbook?.Sheets?.[workbook?.SheetNames?.[0]];
  const rows = XLSX?.utils?.sheet_to_json ? XLSX.utils.sheet_to_json(sheet, { defval: "" }) : [];
  return rows
    .map((row) => Object.entries(row || {}).reduce((acc, [key, value]) => {
      acc[clean(key)] = normalizeSpreadsheetCell(value);
      return acc;
    }, {}))
...
async function fetchNedarimTransactions(connection, { dateFrom, dateTo }) {
  ...
  for (const method of ["POST", "GET"]) {
    try {
      ...
      const rows = (await parseCsvWithXlsx(buffer)).map((row) => mapNedarimTransaction(connection, row));
