...
async function stripeDeleteRequest(connection, path) {
  const auth = Buffer.from(`${clean(connection.secret)}:`).toString("base64");
...
export async function cancelPaymentMandate({
  connectionId,
  mandateId
}) {
  const connection = await getPaymentConnectionById(connectionId, { includeSecret: true });
  ...
  if (connection.provider !== "stripe") {
    throw new Error("מחיקת הוראת קבע זמינה כרגע רק עבור Stripe.");
  }
  ...
  const canceled = await stripeDeleteRequest(connection, `/v1/subscriptions/${encodeURIComponent(safeMandateId)}`);
  return {
    ok: true,
    mandateId: safeMandateId,
    status: clean(canceled?.status) || "canceled"
  };
}
