...
async function fetchNedarimMandates(connection) {
  ...
  return allMandates
    ...
    .filter((mandate) => mandate && ["active", "issues"].includes(mandate.status));
}
...
async function fetchStripeMandates(connection) {
  ...
  do {
    ...
    chunk.forEach((subscription) => {
      if (["canceled", "incomplete_expired"].includes(cleanLower(subscription?.status))) return;
      const mapped = mapStripeMandate(connection, subscription);
      if (!mapped || !["active", "issues"].includes(mapped.status)) return;
