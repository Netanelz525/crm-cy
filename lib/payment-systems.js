...
function parseCsv(text) {
  ...
  return dataRows.map((row) => (
    headerRow.reduce((acc, header, index) => {
      acc[clean(header)] = clean(row[index]);
      return acc;
    }, {})
  ));
}
...
function mapNedarimTransaction(connection, row) {
  const dateText = findRowValue(row, ["תאריך", "Date", "תאריך עסקה", "DateTime", "CreatedAt"]);
  const amountText = findRowValue(row, ["סכום", "Amount", "AmountTotal", "Total", "Sum"]);
  const amount = normalizeAmount(amountText);
  let createdAtUnix = Date.parse(dateText) || 0;
  if (!createdAtUnix && /^\d{1,2}\/\d{1,2}\/\d{4}/.test(dateText)) {
    const [datePart, timePart = "00:00:00"] = dateText.split(" ");
    const [day, month, year] = datePart.split("/");
    createdAtUnix = Date.parse(`${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}T${timePart}`) || 0;
  }
  return {
    id: clean(findRowValue(row, ["מספר עסקה", "TransactionId", "Reference", "אסמכתא"])) || randomUUID(),
    provider: "nederim",
    providerLabel: providerLabel("nederim"),
    connectionId: connection.id,
    connectionLabel: connection.label,
    externalId: connection.external_id,
    currency: normalizeCurrency(findRowValue(row, ["מטבע", "Currency"]) || "ILS"),
    amount,
    netAmount: amount,
    feeAmount: 0,
    status: findRowValue(row, ["סטטוס", "Status", "מצב"]),
    description: findRowValue(row, ["תיאור", "Description", "עבור", "פרטים"]),
    customerName: findRowValue(row, ["שם", "Name", "לקוח", "תורם"]),
    reference: findRowValue(row, ["אסמכתא", "Reference", "Invoice", "מספר אישור"]),
    type: findRowValue(row, ["סוג", "Type"]),
    createdAt: dateText,
    createdAtUnix,
    raw: row
  };
...
function mapStripeTransaction(connection, transaction) {
  return {
    ...
    customerName: "",
    reference: clean(transaction?.source || transaction?.id),
    type: clean(transaction?.type),
...
async function fetchStripeTransactions(connection, { dateFrom, dateTo }) {
  ...
  do {
    ...
    chunk.forEach((item) => allTransactions.push(mapStripeTransaction(connection, item)));
