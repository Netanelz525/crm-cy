...
export async function sendResendEmail({ to, subject, text, html, replyTo, from, attachments = [] }) {
  const config = getConfig();
  ...
  if (!response.ok) {
    throw new Error(data?.message || data?.error?.message || "שליחת המייל דרך Resend נכשלה");
  }
