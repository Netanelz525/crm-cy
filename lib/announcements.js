...
async function ensureAnnouncementTemplateCatalog() {
  ...
  for (const template of ANNOUNCEMENT_TEMPLATE_CATALOG) {
    const existingRows = await sql`
      SELECT id, name, template_key, generator_name, google_docs_url, google_docs_id, category, version, engine, active, fields_json, allowed_roles
...
export async function listAnnouncementTemplates({ user = null } = {}) {
  ...
  const rows = await sql`
    SELECT id, name, template_key, generator_name, google_docs_url, google_docs_id, category, version, engine, active, fields_json, allowed_roles, header_text, footer_text, blank_object_key, blank_content_type, layout_json, created_by_user_id, created_at, updated_at
    FROM announcement_templates
    WHERE active = TRUE
