...
export async function searchStudentsByTz(tznum, options = {}) {
  if (!tznum) return [];
  const data = await gql(STUDENTS_BY_TZ_QUERY, { tznum, first: 50 });
  const edges = data?.students?.edges || [];
  return filterSoftDeletedStudents(edges.map((e) => mapNode(e.node)), options);
}

export async function getStudentsByInstitution(institution, options = {}) {
  ...
  return filterSoftDeletedStudents(out, options);
}

export async function listAllStudents(limitPerPage = 200, maxPages = 10, options = {}) {
  const out = [];
  let after = null;
  let hasNextPage = true;
  let page = 0;
...
export async function getStudentById(id, options = {}) {
  if (!options.includeSoftDeleted && await isSoftDeletedStudentId(id)) return null;
...
export async function getStudentByPrimaryEmail(email, options = {}) {
  ...
  try {
    ...
    const visible = await filterSoftDeletedStudents([first?.node].filter(Boolean).map((node) => mapNode(node)), options);
