...
import { canUseColorPrint, createPrintJobFromBuffer } from "./print-jobs";
...
export async function createPrintJobFromStoredDocument({ storedDocument, user, copies = 1, printPlan = "booklet-bw" }) {
...
export function buildDocumentAnalysisReply({ documentInfo, students }) {
  ...
  if (matchedStudent) {
    lines.push("", `התאמה ב-CRM: ${matchedStudent.label}`, "לא בוצע שינוי עדיין. בחר אם לשייך את הצילום בלבד, או לשייך ולעדכן את השדות שזוהו.");
...
function buildDocumentWorkflowReply(storedDocument, { canLinkStudentDocuments = false, printPlans = DOCUMENT_WORKFLOW_PRINT_PLANS } = {}) {
  const lines = [
    `קיבלתי את המסמך: ${clean(storedDocument?.fileName) || "ללא שם"}`,
    "",
    "ברירת המחדל היא הדפסה בשחור לבן.",
    "בחר תוכנית הדפסה מהתפריט שנפתח. לאחר הבחירה יישלח עותק אחד, ותוכל להוסיף עותקים נוספים.",
    "למשתמשי קרדיט, קבצי Word או Excel חייבים קודם לעבור המרה ל-PDF בעלות 2 עמודי שימוש."
  ];
  if (canLinkStudentDocuments) {
    lines.push("לסופר־אדמין זמינה גם אפשרות נפרדת לשיוך המסמך לתלמיד.");
  }
  return lines.join("\n");
}

function getDocumentWorkflowPrintPlans(user) {
  return DOCUMENT_WORKFLOW_PRINT_PLANS.filter((plan) => !plan.value.endsWith("-color") || canUseColorPrint(user));
}
...
export async function processDocumentAttachment({ user, attachment, messageText = "", source = "web" }) {
  ...
  const pendingAction = students.length === 1 || students.length === 0 ? {
    ...
    storedDocument,
    documentInfo,
    updateStudentData: students.length === 1 ? buildCreateStudentDataFromDocument(documentInfo) : {},
