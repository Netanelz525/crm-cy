...
function pickResources(resource) {
  ...
  if (!normalized || normalized === "all") {
    return ["app_users", "student_internal_notes", "saved_student_views", "neon_user_preferences", "neon_students", "api_tokens"];
  }
  return [normalized];
}
...
export async function exportCrmData(resource = "all") {
  ...
  if (resources.includes("neon_user_preferences")) {
    const rows = await sql`SELECT * FROM neon_user_preferences ORDER BY updated_at DESC`;
    data.neon_user_preferences = rows;
    counts.neon_user_preferences = rows.length;
  }

  if (resources.includes("neon_students")) {
    const rows = await sql`SELECT * FROM neon_students ORDER BY COALESCE(last_name, ''), COALESCE(first_name, ''), full_name ASC`;
