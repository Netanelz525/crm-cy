...
export async function sendAttendanceSessionWhatsAppApprovedTemplate({
  ...
  const isInvitationTemplate = clean(template.name).startsWith("general_meeting_invitation");
...
function templateBodyParameterCount(template) {
  ...
  return explicitCount || (clean(template?.name).startsWith("general_meeting_invitation") ? 3 : 0);
}
...
export function normalizeCoexistenceTemplate(row) {
  ...
  const parameterCount = providerParameterCount || (clean(row?.name).startsWith("general_meeting_invitation") ? 3 : 0);
