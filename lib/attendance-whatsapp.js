...
function templateParameters(template, context, parameterMappings = []) {
  const count = Math.max(0, Number(template?.bodyParameterCount ?? template?.parameterCount) || 0);
  ...
  for (const rosterStudent of students) {
    ...
    for (const recipient of recipients) {
      try {
        ...
        const expectedBodyParameterCount = Math.max(0, Number(template.bodyParameterCount ?? template.parameterCount) || 0);
        ...
        console.info("Attendance WhatsApp template payload prepared", {
          ...
          hasHeaderMedia: Boolean(template.requiresMedia)
        });
        await sendTemplate({
          messaging_product: "whatsapp",
          recipient_type: "individual",
...
function componentText(component) {
  return clean(component?.text || component?.bodyText || component?.body_text);
}
...
export function normalizeCoexistenceTemplate(row) {
  ...
  const parameterCount = inferredCount > 0
    ? inferredCount
    : (bodyExplicitCount ?? (exampleCount > 0 ? exampleCount : explicitTemplateParameterCount(row) ?? 0));
  ...
  return {
    ...
    parameterFormat,
    components,
    headerFormat: clean(header?.format || header?.format_type).toUpperCase(),
    requiresImage: clean(header?.format || header?.format_type).toUpperCase() === "IMAGE",
    requiresMedia: ["IMAGE", "DOCUMENT"].includes(clean(header?.format || header?.format_type).toUpperCase()),
