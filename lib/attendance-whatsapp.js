...
export async function sendAttendanceSessionWhatsApp({
  ...
  const isParentAudience = roster.session.communicationAudience === "parents";
    ...
    for (const recipient of recipients) {
      ...
      try {
        await sendTemplate({
          ...
          to: recipient.value,
          type: "template",
          template: { name: isParentAudience ? PARENT_TEMPLATE_NAME : TEMPLATE_NAME, language: { code: TEMPLATE_LANGUAGE }, components }
        });
        sent += 1;
      } catch (error) {
        failed += 1;
