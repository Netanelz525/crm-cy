...
function mappedTemplateValue(mapping, { recipient, student, session, statusLabel, responseStatusLabels = [] }) {
  ...
  if (source === "recipient_name") return clean(recipient?.label) || "נמען";
  if (source === "student_name") return clean(student?.label) || "תלמיד";
  if (source === "class") return clean(student?.class) || "-";
  if (source === "meeting_title") return clean(session?.displayTitle || session?.title || session?.sessionTypeLabel) || "מפגש";
  if (source === "meeting_date") return formatIsraeliDate(session?.sessionDate) || "-";
  if (source === "attendance_status") return clean(statusLabel) || "-";
  if (source === "response_status_1") return clean(responseStatusLabels[0]) || "-";
  if (source === "response_status_2") return clean(responseStatusLabels[1]) || "-";
...
export async function sendAttendanceSessionWhatsAppApprovedTemplate({
  ...
  if (template.requiresImage && !clean(imageId)) throw new Error("התבנית שנבחרה דורשת תמונה.");
  const labels = buildAttendanceStatusLabels(roster.session.customStatuses);
  const statuses = unique(responseStatuses).map(normalizeAttendanceStatus).filter((value) => labels[value]);
  if (statuses.length !== 2) throw new Error("יש לבחור בדיוק שני סטטוסים לעדכון מצב הנוכחות מתוך WhatsApp.");
  ...
  if (quickReplyButtons.length < 2) throw new Error("התבנית שנבחרה חייבת לכלול שני כפתורי תשובה מהירה לעדכון נוכחות.");
    ...
    for (const recipient of recipients) {
      try {
        ...
        await sendWhatsAppTemplateMessage(recipient.value, {
          ...
          bodyParameters: templateParameters(template, {
            recipient,
            student,
          ...
          components: [
            ...(template.requiresImage ? [{
              type: "header",
              parameters: [{ type: "image", image: { id: clean(imageId) } }]
            }] : []),
            ...(Number(template.parameterCount) > 0 ? [{
              ...
              parameters: templateParameters(template, {
                ...
                statusLabel: labels[normalizeAttendanceStatus(rosterStudent.status)] || normalizeAttendanceStatus(rosterStudent.status),
                responseStatusLabels: statuses.map((status) => labels[status])
              }, parameterMappings).map((text) => ({ type: "text", text }))
            }] : []),
            { type: "button", sub_type: "quick_reply", index: "0", parameters: [{ type: "payload", payload: responsePayloads[0] }] },
            { type: "button", sub_type: "quick_reply", index: "1", parameters: [{ type: "payload", payload: responsePayloads[1] }] }
          ]
        });
        sentMessages += 1;
      } catch (error) {
