...
export function buildImportReportWorkbook({ fileName, matchMapping = {}, fieldMapping = {}, rowResults = [] } = {}) {
  ...
  const matchSummary = Object.entries(matchMapping)
    .filter(([, header]) => clean(header))
    .map(([field, header]) => `${field}: ${header}`)
    .join(" | ");
  const fieldSummary = Object.entries(fieldMapping)
    .filter(([, header]) => clean(header))
    .map(([field, header]) => `${field}: ${header}`)
    .join(" | ");
  ...
  const summaryRows = [
    ...
    { פריט: "סה\"כ שורות", ערך: rowResults.length },
    { פריט: "עודכנו", ערך: rowResults.filter((row) => row.status === "updated").length },
    { פריט: "דולגו", ערך: rowResults.filter((row) => row.status === "skipped").length },
    { פריט: "נכשלו", ערך: rowResults.filter((row) => row.status === "failed").length }
  ];

  const reportRows = rowResults.map((result) => ({
    שורה: result.rowNumber,
    סטטוס: result.outcome,
    פירוט: clean(result.message),
    "מזהה תלמיד": clean(result.studentId),
    "שם תלמיד": clean(result.studentName),
    ...(result.row || {})
  }));
