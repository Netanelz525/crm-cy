...
import { getTaskById, listOfficeTaskEmailUsers, taskStatusLabel } from "./tasks.js";
...
async function recipientsForTask(task) {
  const assigneeIds = new Set((task?.assignees || []).map((user) => clean(user.id)).filter(Boolean));
  if (!assigneeIds.size) return [];
  const officeUsers = await listOfficeTaskEmailUsers();
  const emails = officeUsers
    .filter((user) => assigneeIds.has(clean(user.id)))
