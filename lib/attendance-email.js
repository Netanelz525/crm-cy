...
export async function sendAttendanceInvitationEmails({
  ...
  const subjectText = clean(subject || session.invitationEmailSubject) || `הזמנה ל${clean(session.displayTitle || session.title || "אירוע")}`;
  const messageText = clean(body || session.invitationEmailBody);
