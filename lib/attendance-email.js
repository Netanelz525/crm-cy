...
function formatCurrentHebrewDate() {
  try {
...
function withSendHebrewDate(subject) {
  const baseSubject = clean(subject);
  const hebrewDate = clean(formatCurrentHebrewDate());
  if (!baseSubject || !hebrewDate) return baseSubject;
  if (baseSubject.includes(hebrewDate)) return baseSubject;
  return `${baseSubject} | ${hebrewDate}`;
}
...
export async function createAttendanceResponseToken({ sessionId, studentId, recipientEmail, recipientName, recipientRole, status, createdByUserId }) {
  const id = randomUUID();
...
export async function sendAttendanceSessionEmails({
  ...
  let failedEmails = 0;
  const subject = withSendHebrewDate(subjectText || buildAttendanceEmailSubject(roster.session));
  ...
  for (const item of rawRecipients) {
    ...
    for (const status of responseStatuses) {
      const tokenId = await createAttendanceResponseToken({
        sessionId: roster.session.id,
...
export async function sendAttendanceInvitationEmails({
  sessionId,
  studentId = "",
  subject = "",
  body = "",
  ...
  const subjectText = clean(subject || session.invitationEmailSubject || session.displayTitle || session.title) || `הזמנה ל${clean(session.displayTitle || session.title || "אירוע")}`;
  const messageText = clean(body || session.invitationMessage || session.invitationEmailBody);
  if (!messageText) throw new Error("יש להזין תוכן הזמנה במייל.");
  const normalizedRoles = normalizeRecipientRoles(recipientRoles.length ? recipientRoles : session.invitationEmailRecipientRoles);
  if (!normalizedRoles.length) throw new Error("יש לבחור לפחות סוג נמען אחד להזמנה.");
  const selectedRosterStudents = roster.students.filter((student) => !clean(studentId) || clean(student.id) === clean(studentId));
  if (!selectedRosterStudents.length) throw new Error("התלמיד אינו נמצא במפגש.");
  const fullStudents = (await Promise.all(selectedRosterStudents.map((student) => getNeonStudentById(student.id)))).filter(Boolean);
  const rawRecipients = [];
  for (const student of fullStudents) {
    for (const recipient of buildRecipientsForStudent(student, normalizedRoles)) {
      rawRecipients.push({ student, recipient });
    }
  }
  const blockedEmails = await getUnsubscribedEmailSet(rawRecipients.map((item) => item.recipient.email));
  let sentEmails = 0;
  ...
  for (const item of rawRecipients) {
    ...
    const parts = buildPreviewMessageParts({
      subject: withSendHebrewDate(subjectText),
      bodyText: messageText,
      bodyHtml: "",
      includeGreeting: true,
      recipientName: item.recipient.name || item.recipient.email,
    ...
    try {
      await sendResendEmail({
        ...
        subject: parts.subject,
        text: parts.text,
        html: renderEmailHtml({ subject: parts.subject, html: parts.html, trustedHtml: true }),
        from: buildResendFromAddress("הזמנת אירוע"),
        attachments: attachment?.content ? [{
          filename: clean(attachment.fileName) || "attachment",
          content: attachment.content,
          content_type: clean(attachment.contentType)
        }] : []
      });
      sentEmails += 1;
    } catch (error) {
      console.error("Attendance invitation email failed", { sessionId, studentId: item.student.id, error: clean(error?.message) });
    }
  }
  return { ok: true, matchedStudents: fullStudents.length, sentEmails, skippedBlacklisted, missingRecipients };
}
