...
function extractAddressUpdateValue(text) {
  ...
  const patterns = [
    ...
    /(?:תעדכן|עדכן)\s+(?:לו|לה|לו את|לה את|את)?\s*(?:הכתובת|כתובת)\s+(.*)$/iu
  ];
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  const contextualUpdateStudentId = clean(recentMentionedStudentContext?.studentId || recentStudentContext?.studentId);
  ...
  const systemPrompt = {
    ...
    content: [
      ...
      contextualUpdateStudentId && intentType === "update_request"
        ? `המשתמש מבקש לעדכן את התלמיד האחרון שזוהה בשיחה: ${clean(recentMentionedStudentContext?.studentName || recentStudentContext?.studentName)} (${contextualUpdateStudentId}). אם יש מספיק פרטי עדכון, עבור ישר ל-propose_update_student.`
        : "",
    ...
    if (Array.isArray(assistantMessage.tool_calls) && assistantMessage.tool_calls.length) {
      ...
      for (const toolCall of assistantMessage.tool_calls) {
        const result = await executeToolCall(toolCall, {
          lastUserMessage,
          intentType,
          recentStudentId: recentMentionedStudentContext?.studentId || recentStudentContext?.studentId || ""
        });
