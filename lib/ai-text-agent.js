...
function extractStudentNameFromCreateText(text) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      return {
        firstName: clean(match[1]),
        lastName: clean(match[2]).replace(/\s{2,}/g, " ")
      };
    }
  }
...
function extractStudentCreateDataFromText(text) {
  ...
  return normalizeStudentInput({
    "fullName.firstName": firstName || inferredName.firstName || derivedFirstName,
    "fullName.lastName": lastName || inferredName.lastName || derivedLastName,
    ...
    currentInstitution: extractLabeledValue(raw, ["מוסד נוכחי", "מוסד", "ישיבה"]) || extractLooseLabeledValue(raw, ["מוסד נוכחי", "מוסד", "ישיבה"]),
    class: extractLabeledValue(raw, ["כיתה", "שיעור"]) || extractLooseLabeledValue(raw, ["כיתה", "שיעור"]),
    registration: extractLabeledValue(raw, ["רישום"]) || extractLooseLabeledValue(raw, ["רישום"]),
    famliystatus: extractLabeledValue(raw, ["סטטוס משפחתי", "מצב משפחתי"]) || extractLooseLabeledValue(raw, ["סטטוס משפחתי", "מצב משפחתי"]),
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  if (intentType !== "create_request" && intentType !== "update_request" && (quantitativeListRequest || choiceFieldQuery)) {
    ...
    const assistantMessage = await createAiChatMessage({
      ...
      role: "assistant",
      content: reply,
      metadata: { studentCards: finalStudentCards, exportUrl, pdfUrl, viewUrl, intentType, path: "deterministic", resultCount: students.length, searchSummary, source }
    });

    return { ...assistantMessage, reply, exportUrl, pdfUrl, viewUrl, searchSummary, pendingAction: null };
  }
  ...
  if (intentType === "create_request" && !finalPendingAction) {
    const createContextText = [...recentConversation.map((item) => item.content), lastUserMessage].filter(Boolean).join("\n");
  ...
  const assistantSaved = await createAiChatMessage({
    ...
    metadata: { studentCards: finalStudentCards, exportUrl, pdfUrl, viewUrl, intentType, path: "tool", resultCount: finalStudentCards.length, searchSummary, pendingAction: finalPendingAction, source }
  });
