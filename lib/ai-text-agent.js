...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  const assistantSaved = await createAiChatMessage({
    ...
    metadata: { studentCards: finalStudentCards, exportUrl, viewUrl, intentType, path: "tool", resultCount: finalStudentCards.length, searchSummary, pendingAction: finalPendingAction, source }
  });
