...
function buildQuantitativeReply({ students, requestedLimit, viewUrl, totalCount = null }) {
  const total = Number.isFinite(Number(totalCount)) ? Number(totalCount) : students.length;
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  if (missingDataCategoryQuery) {
    ...
    const exportUrl = categorySupportsMissingType && effectiveFilters.length
      ...
      : "";
    const pdfUrl = categorySupportsMissingType && effectiveFilters.length && isInstitutionLevelQuery({ query: lastUserMessage, filters: effectiveFilters })
      ? buildInstitutionPdfUrlForFilters(effectiveFilters, { missingType: missingDataCategoryQuery.type })
      : "";
    const sortLevels = exportUrl || pdfUrl ? defaultReportSortLevels() : [];
