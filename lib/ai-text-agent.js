...
import { listTodayAttendanceSessionsForAgent } from "./attendance";
import {
  buildStudentSummary,
...
import { maybeBuildPaymentReportAgentResult } from "./payment-agent";

const OPENAI_API_KEY = process.env.OPENAI_API_KEY;
const OPENAI_MODEL = process.env.OPENAI_MODEL || "gpt-4.1-mini";
export const CRM_SCOPE_MESSAGE = "אני כאן כדי לעזור בנושאי CRM ומערכות התשלום: תלמידים, שדות, סטטוסים, מסמכים, פעולות עבודה ודוחות תרומות/עסקאות. אם תרצה, אפשר פשוט לנסח את השאלה מחדש בהקשר של תלמידים, CRM או תשלומים.";
...
function isAttendanceTodayQuery(text) {
  const raw = clean(text);
  if (!raw) return false;
  const mentionsAttendance = ["נוכחות", "מפגש", "מפגשים", "ביקורת"].some((pattern) => raw.includes(pattern));
  const mentionsToday = ["היום", "יומי", "של היום", "מה מצב"].some((pattern) => raw.includes(pattern));
  return mentionsAttendance && mentionsToday;
}

function currentIsraelDateInput() {
...
function buildAttendanceTodayReply(sessions) {
  if (!sessions.length) {
    return `לא מצאתי מפגשי נוכחות להיום (${currentIsraelDateInput()}).`;
  }

  return sessions.map((session) => {
    const lines = [
      `עדכון נוכחות: ${session.title}`,
      `תאריך: ${session.sessionDate}`
    ];

    if (session.recentFoundNames.length) {
      lines.push("נוספו כנוכחים:");
      session.recentFoundNames.forEach((name) => {
        lines.push(`- ${name}`);
      });
    }

    lines.push(`נוכחים: ${session.foundCount}/${session.totalStudents}. חוסר: ${session.missingPercent}% (${session.missingCount}/${session.totalStudents}).`);
    return lines.join("\n");
  }).join("\n\n");
}

function buildAttendanceTodayActionLinks(sessions) {
  const links = [];
  for (const session of sessions) {
    links.push({ label: `פתח מפגש: ${session.title}`, url: session.sessionUrl });
    links.push({ label: `PDF ${session.title} - שיעור ואז משפחה`, url: session.pdfUrls.className });
    links.push({ label: `PDF ${session.title} - לפי סטטוס נוכחות`, url: session.pdfUrls.status });
  }
  return links;
}
...
const MISSING_DATA_CATEGORY_DEFINITIONS = [
  {
    key: "contact",
    label: "פרטי הורים",
    missingType: "contact",
    patterns: [
      ...
      "פרטי קשר להורים",
      "פרטי קשר הורים",
      "טלפון ואימייל של הורה",
      "טלפון ואימייל של ההורים"
    ]
  },
  {
    key: "father",
  ...
  ...item,
  normalizedPatterns: item.patterns.map((pattern) => normalizeContextText(pattern))
}));

function getMissingDataCategoryQueryInfo(text) {
  const raw = clean(text);
  if (!raw || !/(?:חסר|חסרה|חסרים|חסרות|חוסר|אין|בלי)/u.test(raw)) return null;

  const normalized = normalizeContextText(raw);
  const matchedCategory = MISSING_DATA_CATEGORY_DEFINITIONS.find((candidate) =>
    candidate.normalizedPatterns.some((pattern) => normalized.includes(pattern))
  );
  if (!matchedCategory) return null;

  const wantsCount = /(?:^|\s)(?:כמה|לכמה)\b/u.test(raw);
  const wantsList = /(?:תרשום|תכתוב|תראה|הצג|תציג|מי|למי|איזה|לאיזה)/u.test(raw);

  return {
    key: matchedCategory.key,
    type: clean(matchedCategory.missingType),
    label: matchedCategory.label,
    wantsList: wantsCount ? false : (wantsList || true)
  };
}

function studentMatchesMissingDataCategory(student, categoryKey) {
  const key = clean(categoryKey);
  if (!student || !key) return false;

  const hasPhone = (value) => Boolean(clean(value?.primaryPhoneNumber));
  const hasEmail = (value) => Boolean(clean(value?.primaryEmail));

  switch (key) {
    case "contact":
      return !(
        (hasPhone(student?.dadPhone) && hasEmail(student?.fatherEmail))
        || (hasPhone(student?.momPhone) && hasEmail(student?.motherEmail))
      );
    case "father":
      return !(
        clean(student?.shmHb)
        && clean(student?.tzaba)
        && hasPhone(student?.dadPhone)
        && hasEmail(student?.fatherEmail)
      );
    case "mother":
      return !(
        clean(student?.shmHm)
        && clean(student?.tzMotherNum)
        && hasPhone(student?.momPhone)
        && hasEmail(student?.motherEmail)
      );
    case "bank":
      return !(clean(student?.bankNum) && clean(student?.senif) && clean(student?.accountNum));
    case "identity":
      return !(clean(student?.tznum) && clean(student?.dateofbirth));
    case "student_contact":
      return !(hasPhone(student?.phone) && hasEmail(student?.email));
    case "address":
      return !(clean(student?.adders?.addressStreet1) && clean(student?.adders?.addressCity));
    default:
      return false;
  }
}
...
function isCrmRelevant(text) {
  ...
  return [
    ...
    "תרומה", "תרומות", "עסקה", "עסקאות", "תשלום", "תשלומים", "סטרייפ", "stripe", "נדרים", "דוח תרומות", "דוח עסקאות"
  ].some((pattern) => raw.includes(pattern));
}
...
function buildQuantitativeReply({ students, requestedLimit, viewUrl, totalCount = null }) {
  const safeStudents = Array.isArray(students) ? students.filter(Boolean) : [];
  const total = Number.isFinite(Number(totalCount)) ? Number(totalCount) : safeStudents.length;
  if (!total) return "לא נמצאו תלמידים מתאימים.";
  const displayLimit = requestedLimit || Math.min(total, 200);
  const displayed = safeStudents.slice(0, displayLimit);
  const lines = [
    `נמצאו ${total} תלמידים.`,
...
function buildDeterministicStudentListReply({ students, requestedLimit, viewUrl, totalCount = null }) {
  const safeStudents = Array.isArray(students) ? students.filter(Boolean) : [];
  const total = Number.isFinite(Number(totalCount)) ? Number(totalCount) : safeStudents.length;
  if (!total) return "לא נמצאו תלמידים מתאימים.";
  ...
  if (!requestedLimit && viewUrl) {
    lines.push("אפשר לפתוח את המסך המלא כדי לבחור תלמידים, לעדכן שדות בצורה מרוכזת או לייצא לאקסל/PDF.");
  }
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  if (isAttendanceTodayQuery(lastUserMessage)) {
    const sessions = await listTodayAttendanceSessionsForAgent({ date: currentIsraelDateInput() });
    const reply = buildAttendanceTodayReply(sessions);
    const actionLinks = buildAttendanceTodayActionLinks(sessions);
    const searchSummary = `נשלף מצב מפגשי הנוכחות של היום (${currentIsraelDateInput()})`;

    await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "user",
      content: lastUserMessage,
      metadata: { intentType: "attendance_today", path: "attendance_today", source }
    });
    const assistantSaved = await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "assistant",
      ...
      metadata: {
        ...
        pdfUrl: "",
        actionLinks,
        viewUrl: sessions[0]?.sessionUrl || "",
        intentType: "attendance_today",
  ...
  const paymentReportResult = await maybeBuildPaymentReportAgentResult({
    user,
    messageText: lastUserMessage,
    source
  });
  if (paymentReportResult) {
    await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "user",
      content: lastUserMessage,
      metadata: { intentType: "payment_report", path: "payment_report", source }
    });
    const assistantSaved = await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "assistant",
      content: paymentReportResult.reply,
      metadata: {
        studentCards: [],
        exportUrl: paymentReportResult.exportUrl,
        pdfUrl: paymentReportResult.pdfUrl,
        viewUrl: paymentReportResult.viewUrl,
        intentType: "payment_report",
        path: "payment_report",
        resultCount: 0,
        searchSummary: paymentReportResult.searchSummary,
        paymentReportConfig: paymentReportResult.paymentReportConfig,
        paymentSummary: paymentReportResult.paymentSummary,
        source
      }
    });
    ...
    return {
      ...assistantSaved,
      reply: paymentReportResult.reply,
      studentCards: [],
      exportUrl: paymentReportResult.exportUrl,
      pdfUrl: paymentReportResult.pdfUrl,
      viewUrl: paymentReportResult.viewUrl,
  ...
  const missingDataCategoryQuery = getMissingDataCategoryQueryInfo(lastUserMessage);
  ...
  if (missingFieldQuery) {
    ...
    const pdfUrl = effectiveFilters.length
  ...
  if (missingDataCategoryQuery) {
    const categorySupportsMissingType = ["contact", "identity"].includes(missingDataCategoryQuery.type);
    const baseResult = categorySupportsMissingType
      ? await findStudentsMissingDataForAgent({
          type: missingDataCategoryQuery.type,
          query: lastUserMessage,
          filters: inferredChoiceFilters,
          limit: requestedLimit || 200
        })
      : await findStudentsForAgent({
          query: inferredChoiceFilters.length ? "" : lastUserMessage,
          filters: inferredChoiceFilters,
          minScore: 0.4
        });
    const categoryStudents = categorySupportsMissingType
      ? baseResult.students
      : baseResult.students.filter((student) => studentMatchesMissingDataCategory(student, missingDataCategoryQuery.key));
    const effectiveFilters = Array.isArray(baseResult.effectiveFilters) ? baseResult.effectiveFilters : [];
    const exportUrl = categorySupportsMissingType && effectiveFilters.length
      ? buildExportUrlForFilters(effectiveFilters, { missingType: missingDataCategoryQuery.type })
      : "";
    const pdfUrl = effectiveFilters.length
      ? buildInstitutionPdfUrlForFilters(
          effectiveFilters,
          categorySupportsMissingType ? { missingType: missingDataCategoryQuery.type } : {}
        )
      : "";
    const sortLevels = exportUrl || pdfUrl ? defaultReportSortLevels() : [];
    const viewUrl = categorySupportsMissingType
      ? buildNeonViewUrlForAgent({
          filters: effectiveFilters,
          missingType: missingDataCategoryQuery.type
        })
      : "";
    const describedFilters = describeAgentFilters(effectiveFilters);
    const scopeText = describedFilters.length ? ` מתוך הסינון: ${describedFilters.join(" | ")}` : "";
    const reply = missingDataCategoryQuery.wantsList
      ? buildQuantitativeReply({
          students: categoryStudents,
          requestedLimit,
          viewUrl,
          totalCount: categoryStudents.length
        })
      : `ל-${categoryStudents.length} תלמידים חסרים ${missingDataCategoryQuery.label}${scopeText}.`;
    const searchSummary = buildSearchSummary({
      path: "deterministic",
      query: lastUserMessage,
      filters: effectiveFilters,
      resultCount: categoryStudents.length
    });
    await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "user",
      content: lastUserMessage,
      metadata: { intentType, path: "deterministic_missing_category", source }
    });
    const assistantMessage = await createAiChatMessage({
      clerkUserId: user.clerk_user_id,
      role: "assistant",
      ...
      metadata: {
        ...
        viewUrl,
        intentType,
        path: "deterministic_missing_category",
        resultCount: categoryStudents.length,
        searchSummary,
        source
      }
  ...
  if (intentType !== "create_request" && intentType !== "update_request" && (quantitativeListRequest || choiceFieldQuery || contextualFilters.length)) {
    ...
    const reply = isStudentCityListQuery(lastUserMessage)
      ...
      : isCityBreakdownQuery(lastUserMessage)
        ...
        : buildDeterministicStudentListReply({
            students,
            requestedLimit,
            viewUrl,
...
export async function handleApprovedAiAction({ user, decision, pendingAction, messageId = "" }) {
  ...
  if (!["approve", "attach_only"].includes(decision)) throw new Error("Invalid decision");

  if (decision === "attach_only" && !["attach_document"].includes(pendingAction.type)) {
    throw new Error("שיוך צילום בלבד זמין רק כאשר נמצא תלמיד קיים.");
  }
  ...
  if (pendingAction.type === "attach_document" || pendingAction.type === "create_student") {
    if (!canLinkDocumentsToStudents(user)) {
      throw new Error("שיוך מסמכים לתלמידים דרך הסוכן זמין רק למנהל או סופר אדמין.");
    }
    ...
    if (decision === "approve" && pendingAction.type === "attach_document") {
      const updateData = pendingAction.updateStudentData || buildCreateStudentDataFromDocument(documentInfo);
      if (Object.keys(updateData).length) {
        const updatedStudent = await updateNeonStudentViaTwenty(studentId, updateData);
        if (!updatedStudent?.id) throw new Error("עדכון פרטי התלמיד נכשל.");
      }
    }
    ...
    const reply = createdStudent
      ...
      : decision === "attach_only" ? "הצילום שויך לכרטיס התלמיד. לא שיניתי שום שדה בכרטיס."
        : "המסמך שויך לכרטיס התלמיד והפרטים שאושרו עודכנו.";
    ...
    await createAiChatMessage({
      ...
      metadata: {
        ...
        searchSummary: createdStudent ? "נוצר תלמיד חדש והמסמך שויך אחרי אישור מפורש" : decision === "attach_only" ? "הצילום שויך ללא שינוי שדות" : "המסמך שויך ושדות התלמיד עודכנו אחרי אישור מפורש"
      }
    });
    return {
