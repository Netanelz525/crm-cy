...
const MISSING_FIELD_ALIASES = [
  { field: "tznum", label: 'ת"ז', patterns: ['ת"ז', "תז", "מספר זהות", "מס זהות", "תעודת זהות"] },
  { field: "dateofbirth", label: "תאריך לידה", patterns: ["תאריך לידה", "ת. לידה", "לידה"] },
  { field: "phone.primaryPhoneNumber", label: "טלפון תלמיד", patterns: ["טלפון תלמיד", "טלפון של התלמיד", "טלפון", "נייד", "פלאפון"] },
  { field: "dadPhone.primaryPhoneNumber", label: "טלפון אב", patterns: ["טלפון אב", "טלפון אבא", "טלפון של האבא", "טלפון של אב"] },
  { field: "momPhone.primaryPhoneNumber", label: "טלפון אם", patterns: ["טלפון אם", "טלפון אמא", "טלפון של האמא", "טלפון של אם"] },
  { field: "email.primaryEmail", label: "אימייל תלמיד", patterns: ["אימייל תלמיד", "מייל תלמיד", "אימייל", "מייל"] },
  { field: "fatherEmail.primaryEmail", label: "אימייל אב", patterns: ["אימייל אב", "מייל אב", "אימייל אבא", "מייל אבא"] },
  { field: "motherEmail.primaryEmail", label: "אימייל אם", patterns: ["אימייל אם", "מייל אם", "אימייל אמא", "מייל אמא"] },
  { field: "adders.addressStreet1", label: "כתובת", patterns: ["כתובת", "כתובת מלאה", "רחוב", "כתובת מגורים"] },
  { field: "adders.addressCity", label: "עיר", patterns: ["עיר", "עיר מגורים"] }
];

function getMissingFieldQueryInfo(text) {
  const raw = clean(text);
  if (!raw || !/(?:חסר|חסרה|חסרים|חסרות|אין|בלי)/u.test(raw) || !/(?:כמה|לכמה)/u.test(raw)) return null;

  for (const candidate of MISSING_FIELD_ALIASES) {
...
function extractAddressUpdateValue(text) {
  ...
  const sanitizeAddressValue = (value) => clean(value)
    ...
    .replace(/^(?:המגורים|כתובת\s+המגורים)\s+/iu, "")
    .replace(/^ל\s+/iu, "")
    .replace(/^(?:היא|זה|הכתובת)\s+/iu, "")
    .replace(/[.]+$/g, "");
  ...
  const patterns = [
    /(?:שדה\s+הכתובת(?:\s+שלו|\s+שלה|\s+של\s+התלמיד)?|כתובת(?:\s+שלו|\s+שלה|\s+של\s+התלמיד)?|רחוב)\s+(.*)$/iu,
    /(?:תעדכן|עדכן)\s+(?:לו|לה|לו את|לה את|את)?\s*(?:הכתובת|כתובת)(?:\s+המגורים)?\s+(.*)$/iu
  ];
...
function extractStudentUpdateDataFromText(text) {
  ...
  const studentPhone = /(טלפון\s+תלמיד|טלפון\s+שלו|הטלפון\s+שלו|נייד\s+שלו|פלאפון\s+שלו|טלפון)/u.test(raw)
    ? extractLabeledValue(raw, ["טלפון תלמיד", "טלפון", "נייד", "פלאפון"]) || extractPhoneDigits(raw)
    : "";
  const dadPhone = /(טלפון\s+אב|טלפון\s+האבא|טלפון\s+של\s+האבא|טלפון\s+של\s+אבא)/u.test(raw)
    ? extractLabeledValue(raw, ["טלפון אב", "טלפון האבא", "טלפון אבא"]) || extractPhoneDigits(raw)
    : "";
  const momPhone = /(טלפון\s+אם|טלפון\s+האמא|טלפון\s+של\s+האמא|טלפון\s+של\s+אמא)/u.test(raw)
  ...
  const normalized = normalizeStudentInput({
    ...
    famliystatus: extractLabeledValue(raw, ["סטטוס משפחתי", "מצב משפחתי"]) || extractLooseLabeledValue(raw, ["סטטוס משפחתי", "מצב משפחתי"]) || enumDefaults.famliystatus,
    "adders.addressStreet1": extractAddressUpdateValue(raw),
    "adders.addressCity": extractLabeledValue(raw, ["עיר"]) || extractLooseLabeledValue(raw, ["עיר"]),
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  if (missingFieldQuery) {
    ...
    const reply = `ל-${students.length} תלמידים חסר השדה ${missingFieldQuery.label}${scopeText}.`;
    const searchSummary = buildSearchSummary({
      path: "deterministic",
      query: lastUserMessage,
