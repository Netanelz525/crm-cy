...
function getMissingDataCategoryQueryInfo(text) {
  ...
  ].map((pattern) => normalizeContextText(pattern));

  if (!contactPatterns.some((pattern) => normalized.includes(pattern))) {
    return null;
  }
...
export async function processTextAiMessage({ user, messageText, source = "web" }) {
  ...
  if (missingDataCategoryQuery) {
    ...
    const exportUrl = result.effectiveFilters.length
      ? buildExportUrlForFilters(result.effectiveFilters, { missingType: missingDataCategoryQuery.type })
      : "";
    const pdfUrl = result.effectiveFilters.length && isInstitutionLevelQuery({ query: lastUserMessage, filters: result.effectiveFilters })
      ? buildInstitutionPdfUrlForFilters(result.effectiveFilters, { missingType: missingDataCategoryQuery.type })
      : "";
    const sortLevels = exportUrl || pdfUrl ? defaultReportSortLevels() : [];
    const viewUrl = buildNeonViewUrlForAgent({
      filters: result.effectiveFilters,
      missingType: missingDataCategoryQuery.type
    });
    const describedFilters = describeAgentFilters(result.effectiveFilters);
    const scopeText = describedFilters.length ? ` מתוך הסינון: ${describedFilters.join(" | ")}` : "";
    const reply = missingDataCategoryQuery.wantsList
      ...
      : `ל-${result.count} תלמידים חסרים ${missingDataCategoryQuery.label}${scopeText}.`;
    const searchSummary = buildSearchSummary({
      path: "deterministic",
      query: lastUserMessage,
      filters: result.effectiveFilters,
