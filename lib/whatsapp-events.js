...
import { extractWhatsAppDeliveryStatuses } from "./whatsapp-delivery-status.mjs";
...
export async function recordWhatsAppDeliveryStatuses(payload, { channel = "operational" } = {}) {
  ...
  for (const item of statuses) {
    await sql`
      INSERT INTO whatsapp_delivery_events (
        id, channel, message_id, recipient_id, delivery_status,
        error_code, error_title, error_message, conversation_id,
        pricing_category, provider_timestamp
      )
      VALUES (
        ${crypto.randomUUID()}, ${clean(channel) || "operational"},
        ${item.messageId || null}, ${item.recipientId || null}, ${item.status},
        ${item.errorCode || null}, ${item.errorTitle || null}, ${item.errorMessage || null},
        ${item.conversationId || null}, ${item.pricingCategory || null},
        ${item.providerTimestamp || null}
      )
    `;
  }
