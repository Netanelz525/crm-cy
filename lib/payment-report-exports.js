...
export async function buildPaymentReportExcelExport(input) {
  ...
  const rows = transactions.map((transaction) => ([
    ...
    Number(transaction.netAmount) || 0,
...
export async function buildPaymentReportPdfExport(input) {
  ...
  const rows = transactions.map((transaction, index) => ({
    ...
    amount: String(Number(transaction.amount || 0).toFixed(2)),
    net: String(Number(transaction.netAmount || 0).toFixed(2)),
  ...
  const pdf = await renderInstitutionPdf({
    title: "דוח עסקאות",
    subtitle: `טווח: ${clean(config.dateFrom)} עד ${clean(config.dateTo)} | עסקאות: ${summary.transactionsCount} | סה״כ: ${summary.totalAmount.toFixed(2)}`,
