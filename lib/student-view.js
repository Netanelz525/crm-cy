...
export const INSTITUTION_COLUMNS = [
  ...
  { key: "fatherTz", label: 'ת"ז אב', defaultSelected: false },
  { key: "motherTz", label: 'ת"ז אם', defaultSelected: false },
  { key: "age", label: "גיל", defaultSelected: true },
  { key: "address", label: "כתובת מלאה", defaultSelected: false },
  { key: "bankDetails", label: "פרטי בנק", defaultSelected: false },
  ...
  { key: "tags", label: "תגיות", defaultSelected: false },
...
export const PDF_PRINT_ONLY_COLUMNS = [
  { key: "blankAttendance", label: "נוכחות", kind: "blank" },
  { key: "blankSignature", label: "חתימה", kind: "blank" },
  { key: "blankManualStatus", label: "סטטוס ידני", kind: "blank" },
  { key: "blankNotes", label: "הערות", kind: "blank" }
];
export const PDF_PRINT_ONLY_COLUMN_MAP = Object.fromEntries(PDF_PRINT_ONLY_COLUMNS.map((column) => [column.key, column]));
...
export const FILTERABLE_FIELDS = [
  ...
  { key: "tznum", label: 'ת"ז' },
  { key: "fatherTz", label: 'ת"ז אב' },
  { key: "motherTz", label: 'ת"ז אם' },
  { key: "age", label: "גיל" },
  { key: "address", label: "כתובת מלאה" },
  { key: "bankDetails", label: "פרטי בנק" },
...
export function parseSortLevels(params) {
  ...
  if (sortBys.length) {
    return sortBys
      .map((sortBy, index) => {
        const normalizedSortBy = clean(sortBy);
        if (!normalizedSortBy) return null;
        return {
  ...
  const fallbackSortBy = clean(params?.sortBy) || "name";
...
function studentAge(student) {
  ...
  return ageOf(student?.dateofbirth);
}
...
function formatBankDetails(student) {
  const bankNumber = clean(student?.bankNum);
  const branchNumber = clean(student?.senif);
  const accountNumber = clean(student?.accountNum);
  const parts = [bankNumber, branchNumber, accountNumber].filter(Boolean);
...
export function columnText(student, columnKey) {
  ...
  switch (columnKey) {
    ...
    case "fatherTz":
      return clean(student?.tzaba) || "-";
    case "motherTz":
      return clean(student?.tzMotherNum) || "-";
    case "age":
      return String(studentAge(student) ?? "-");
    case "address":
      return formatFullAddress(student);
    case "bankDetails":
      return formatBankDetails(student);
    case "studentPhone":
    ...
    case "tags":
      return Array.isArray(student?.tagNames) && student.tagNames.length ? student.tagNames.join(", ") : "-";
    case "macAddress":
...
function comparableValue(student, sortBy) {
  ...
  if (sortBy === "age") return studentAge(student) ?? -1;
...
function compareSingleSort(a, b, sortBy, sortDir = "asc") {
  ...
  const nextSortBy = clean(sortBy) || "name";

  if (!sortBy || nextSortBy === "name") {
...
export function sortStudents(students, sortInput, sortDir = "asc") {
  const levels = Array.isArray(sortInput)
    ...
    : [{ sortBy: clean(sortInput) || "name", sortDir: clean(sortDir).toLowerCase() === "desc" ? "desc" : "asc" }];

  const effectiveLevels = levels.length ? levels : [{ sortBy: "name", sortDir: "asc" }];
...
export function parsePdfBlankColumns(params) {
  const values = parseListParam(params?.pdfBlankCol);
  return values.filter((key) => PDF_PRINT_ONLY_COLUMN_MAP[key]);
}
