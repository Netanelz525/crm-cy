...
export async function createPrintJob({ file, copies = 1, printPlan = DEFAULT_PRINT_PLAN, uploadedByUserId }) {
  await initDb();
...
export async function createPrintJobFromBuffer({ buffer, fileName, contentType = "application/octet-stream", copies = 1, printPlan = DEFAULT_PRINT_PLAN, uploadedByUserId }) {
  await initDb();
