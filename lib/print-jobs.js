...
import { revokeAnnouncementSignatureLinksForPrintJob } from "./announcement-signature-links";
...
export const DEFAULT_PRINT_PLAN = "booklet-bw";
export const DEFAULT_OUTPUT_MODE = "print";
export const CONVERT_PDF_CREDIT_PAGES = 2;
...
export const PRINT_PLAN_LABELS = {
  "booklet-bw": "חוברת שחור לבן, A3, מימין לשמאל, קיפול/הידוק",
  "duplex-bw": "דו צדדי רגיל שחור לבן, A4",
  "corner-staple-bw": "שחור לבן, דו צדדי, הידוק פינה ימנית עליונה",
  "single-a4-bw": "שחור לבן, A4, צד אחד",
  "single-a3-bw": "שחור לבן, A3, צד אחד",
  "booklet-color": "חוברת צבע, A3, מימין לשמאל, קיפול/הידוק",
  "duplex-color": "דו צדדי רגיל צבע, A4",
  "corner-staple-color": "צבע, דו צדדי, הידוק פינה ימנית עליונה",
  "single-a4-color": "צבע, A4, צד אחד",
  "single-a3-color": "צבע, A3, צד אחד",
  "convert-pdf": "המרת Word ל-PDF בלבד"
};
...
function isTemplatePrintJob(job) {
  if (!job) return false;
  ...
  return clean(job.sourceType) === "announcement" && Boolean(template?.googleDocsId || template?.googleDocsUrl || template?.engine === "local-pdf");
}

export function normalizePrintPlan(value) {
  const plan = clean(value);
  const legacyPlans = { booklet: "booklet-bw", duplex: "duplex-bw", "corner-staple": "corner-staple-bw", "corner-staple-rtl": "corner-staple-bw" };
  if (legacyPlans[plan]) return legacyPlans[plan];
  if (Object.prototype.hasOwnProperty.call(PRINT_PLAN_LABELS, plan)) return plan;
  return DEFAULT_PRINT_PLAN;
}

export function isColorPrintPlan(value) {
  return normalizePrintPlan(value).endsWith("-color");
}

export function canUseColorPrint(user) {
  return Boolean(user?.is_super_admin || user?.print_color_enabled === true);
}
...
export function estimatePrintCreditPages({ pageCount, copies = 1, printPlan = DEFAULT_PRINT_PLAN } = {}) {
  ...
  const extraPerCopy = plan.startsWith("corner-staple-") ? 1 : plan.startsWith("booklet-") ? 2 : 0;
  const colorMultiplier = isColorPrintPlan(plan) ? 2 : 1;
  return (safePageCount + extraPerCopy) * safeCopies * colorMultiplier;
}
...
function isWordOrExcelDocument(contentType, fileName) {
...
export async function createPrintJobFromBuffer({
  ...
  if (isColorPrintPlan(normalizedPrintPlan) && !canUseColorPrint(user)) {
    throw new Error("הדפסה בצבע זמינה רק לסופר אדמין ולמשתמשים מורשים.");
  }
  ...
  const requiresConversionBeforeCreditPrint = isWordOrExcelDocument(normalizedContentType, normalizedFileName);
  if (
    normalizedOutputMode === "print" &&
    !unlimitedPrintCredit &&
    requiresConversionBeforeCreditPrint &&
    normalizedPrintPlan !== "convert-pdf"
  ) {
    throw new Error("משתמשים עם קרדיט צריכים קודם לבחור 'המרת קובץ ל-PDF' עבור קבצי Word או Excel. לאחר קבלת הקובץ המומר במייל ניתן לשלוח אותו להדפסה רגילה.");
  }

  const creditPages = normalizedOutputMode !== "print" || unlimitedPrintCredit
    ? 0
    : normalizedPrintPlan === "convert-pdf"
      ? CONVERT_PDF_CREDIT_PAGES
  ...
  if (creditPages > 0) {
    await debitPrintCredit({
      ...
      metadata: {
        ...
        creditReason: normalizedPrintPlan === "convert-pdf" ? "convert-pdf" : "print"
      }
    });
  }
...
export async function deletePrintJob(id) {
  ...
  await revokeAnnouncementSignatureLinksForPrintJob(normalizedId).catch(() => null);
...
export async function failPrintJob(id, { errorMessage = "" } = {}) {
  await initDb();
  const normalizedId = clean(id);
  if (!normalizedId) throw new Error("Missing print job id.");
  const job = await getPrintJobById(normalizedId);
  await sql`
    ...
    SET
      status = 'failed',
      queue_ack_error = ${clean(errorMessage) || "העבודה נכשלה בשרת המקומי"},
      completed_at = NOW()
    WHERE id = ${normalizedId}
      AND status <> 'completed'
  `;
...
export async function sendPrintJobReceiptEmail(jobId) {
  ...
  if (isTemplatePrintJob(job)) return null;
  if (job?.outputMode === "email") return null;
  if (normalizePrintPlan(job?.printPlan) === "convert-pdf") return null;
