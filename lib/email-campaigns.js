...
export async function buildAttachmentsFromForm(formData) {
  ...
  for (const file of files) {
    ...
    if (totalEncodedBytes + ATTACHMENT_OVERHEAD_BUFFER_BYTES > RESEND_MAX_EMAIL_SIZE_BYTES) {
      throw new Error("סך הקבצים המצורפים חורג ממגבלת Resend. אפשר לצרף כמה קבצים, כל עוד הסך הכולל נשאר עד בערך 29MB לפני העלאה.");
    }
    attachments.push({
...
export async function getEmailCampaignDraft(id) {
  ...
  const rows = await sql`
    SELECT id, created_by_user_id, draft_json, created_at, updated_at
