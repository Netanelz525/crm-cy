...
const RESEND_DELIVERY_STATUS_LABELS = {
  queued: "בתור",
  sent: "נשלח ל-Resend",
  delivered: "נמסר",
...
function resendEventStatus(eventType) {
...
function resendEventErrorMessage(eventType, data = {}) {
  const message = clean(data?.error?.message || data?.error_message || data?.message || data?.reason || data?.bounce?.message);
  if (message) return message;
  switch (clean(eventType)) {
    case "email.delivery_delayed":
      return "Resend דיווח שהמסירה מתעכבת.";
    case "email.failed":
      return "Resend דיווח שהשליחה נכשלה.";
    case "email.bounced":
      return "המייל חזר מהשרת של הנמען.";
    case "email.complained":
      return "הנמען דיווח על המייל כספאם.";
    case "email.suppressed":
      return "Resend חסם את השליחה לנמען הזה.";
    default:
      return "";
  }
}
...
function providerStatusLabel(status) {
  return RESEND_DELIVERY_STATUS_LABELS[clean(status)] || clean(status);
}

function normalizeReplyTo(value) {
  const email = normalizeEmail(value);
  if (!email) return getDefaultResendReplyTo();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new Error("כתובת המייל להשבה אינה תקינה.");
  }
  return email;
}
...
function studentName(student) {
  const name = clean(student?.name) || clean(student?.label) || [clean(student?.fullName?.firstName), clean(student?.fullName?.lastName)].filter(Boolean).join(" ") || "ללא שם";
  return [clean(student?.title), name].filter(Boolean).join(" ");
}

function parentName(student, role) {
  if (role === "father") return [clean(student?.fatherTitle), clean(student?.shmHb) || "הורה"].filter(Boolean).join(" ");
  if (role === "mother") return [clean(student?.motherTitle), clean(student?.shmHm) || "הורה"].filter(Boolean).join(" ");
...
export function renderEmailHtml({ subject, html = "", content = "", trustedHtml = false }) {
  const subjectText = escapeHtml(subject || "");
  const safeHtml = trustedHtml ? String(html || "").trim() : normalizeHtml(html);
  const contentHtml = safeHtml || renderPlainTextToHtml(content || "");
  return `<!DOCTYPE html>
...
export async function getEmailCandidateStudents(filters = {}) {
  ...
  let students = [];
  if (q) {
    students = await searchNeonStudentsByText(q, null, 0.35);
  } else {
...
export function normalizeCustomRecipients(input = []) {
  ...
  for (const item of raw) {
    ...
    recipients.push({
      ...
      extraLabel: clean(item?.extraLabel)
    });
  }
...
export function buildDeliveryTargetsFromCustomRecipients(recipients = []) {
  return normalizeCustomRecipients(recipients).map((recipient) => ({
    ...
    extraLabel: recipient.extraLabel
  }));
}
...
function buildMessageParts({ subject, bodyText, bodyHtml, includeGreeting, recipientName, recipientRoleLabel, student }) {
  ...
  const contentHtml = normalizeHtml(personalizedHtml) || renderPlainTextToHtml(personalizedText);

  const finalText = greeting ? `${greeting}\n\n${personalizedText}` : personalizedText;
  const finalHtml = greeting
    ? `<p>${escapeHtml(greeting)}</p>${contentHtml}`
    : contentHtml;
...
function buildCampaignStudentSnapshot(students = []) {
  return (Array.isArray(students) ? students : [])
    .map((student) => ({
      id: clean(student?.id),
      name: studentName(student),
      institution: clean(student?.currentInstitution),
...
export async function sendEmailCampaign({ formData, createdByUserId, permissions = {} }) {
  ...
  const replyTo = normalizeReplyTo(formData.get("replyTo"));
  ...
  const campaignFilterJson = {
    ...filters,
    originalSelectedStudentIds: filters.selectedStudentIds,
    targetStudentIds: targetStudents.map((student) => clean(student.id)).filter(Boolean),
    targetStudentSnapshot: buildCampaignStudentSnapshot(targetStudents)
  };
  ...
  await sql`
    ...
    VALUES (
      ...
      ${JSON.stringify(campaignFilterJson)}::jsonb,
      ${clean(createdByUserId) || null},
      NOW(),
      NOW()
    )
...
export async function sendCustomEmailCampaign({ draft, createdByUserId, permissions = {} }) {
  ...
  const replyTo = normalizeReplyTo(draft?.replyTo);
...
export async function dispatchEmailCampaign({ campaignId, attachments = [], senderName = "", replyTo = "", subject = "", bodyText = "", bodyHtml = "", includeGreeting = true, allowedTargets = [] }) {
  await initDb();
    ...
    try {
      ...
      await sql`
        ...
        SET
          ...
          provider_event_type = COALESCE(provider_event_type, 'email.sent'),
          provider_error = NULL,
          sent_at = NOW(),
          updated_at = NOW()
      ...
      await sql`
        ...
        SET
          ...
          provider_error = ${clean(error?.message)},
          updated_at = NOW()
        WHERE id = ${clean(delivery.id)}
      `;
      failed += 1;
...
export async function listStudentEmailDeliveries(studentId, limit = 12) {
  ...
  return sql`
    SELECT
      ...
      d.status,
      d.provider_status,
      d.provider_event_type,
      d.provider_error,
      d.provider_last_event_at,
      d.error_message,
      d.open_count,
      d.opened_at,
      d.clicked_at,
...
async function refreshEmailCampaignDeliveryCounts(campaignId) {
  const id = clean(campaignId);
  if (!id) return;
  await sql`
    UPDATE email_campaigns
      ...
      status = CASE
        WHEN COALESCE((
         ...
         AND COALESCE((
          ...
          FROM email_deliveries
          WHERE campaign_id = ${id}
            AND status IN ('sent', 'delivered', 'delivery_delayed')
        ), 0) = 0 THEN 'failed'
        WHEN COALESCE((
          SELECT COUNT(*)::int
          FROM email_deliveries
...
export async function recordResendEmailEvent(eventPayload = {}) {
  await initDb();
  const eventType = clean(eventPayload?.type);
  const data = eventPayload?.data && typeof eventPayload.data === "object" ? eventPayload.data : {};
  const providerMessageId = clean(data.email_id || data.id || eventPayload?.email_id);
  const providerStatus = resendEventStatus(eventType);

  if (!eventType || !providerMessageId || !providerStatus) {
    return { ok: false, ignored: true, reason: "unsupported_event", eventType, providerMessageId };
  }

  const providerError = resendEventErrorMessage(eventType, data);
  const isFailure = ["failed", "bounced", "complained", "suppressed"].includes(providerStatus);
  const rows = await sql`
    UPDATE email_deliveries
    SET
      status = ${providerStatus},
      certainty_level = GREATEST(certainty_level, ${EMAIL_CERTAINTY.sent.level}),
      provider_status = ${providerStatus},
      provider_event_type = ${eventType},
      provider_event_json = ${JSON.stringify(eventPayload)}::jsonb,
      provider_error = ${providerError || null},
      error_message = CASE
        WHEN ${isFailure} THEN ${providerError || providerStatusLabel(providerStatus)}
        ELSE error_message
      END,
  ...
  if (!delivery) {
    return { ok: false, ignored: true, reason: "delivery_not_found", eventType, providerMessageId };
  }

  await refreshEmailCampaignDeliveryCounts(delivery.campaign_id);
  return { ok: true, deliveryId: clean(delivery.id), campaignId: clean(delivery.campaign_id), eventType, providerStatus };
}
...
function buildUniqueCampaignStudentRows(deliveries = []) {
  ...
  return Array.from(students.values())
    .map((student) => ({
      מזהה_תלמיד: clean(student.id),
      שם_תלמיד: clean(student.name),
      כתובות_בקמפיין: Array.from(student.emails).join(", "),
      מספר_כתובות: student.emails.size,
      כתובות_שנשלחו: Array.from(student.sentEmails).join(", "),
      מספר_כתובות_שנשלחו: student.sentEmails.size,
      סטטוסים: Array.from(student.statuses).join(", ")
    }))
    .sort((a, b) => clean(a.שם_תלמיד).localeCompare(clean(b.שם_תלמיד), "he"));
}
...
export async function buildEmailCampaignExport(campaignId) {
  ...
  const summaryRows = [
    ...
    ["מייל להשבה", clean(campaign.reply_to)],
  ...
  const deliverySheetRows = deliveries.map((delivery) => ({
    ...
    סטטוס_Resend: providerStatusLabel(delivery.provider_status),
    אירוע_Resend: clean(delivery.provider_event_type),
    שגיאת_Resend: clean(delivery.provider_error),
    עדכון_אחרון_Resend: formatExportDate(delivery.provider_last_event_at),
    ...
    מייל_להשבה: clean(delivery.reply_to),
  ...
  const studentSheetRows = buildUniqueCampaignStudentRows(deliveries);
  XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(studentSheetRows), "תלמידים");
