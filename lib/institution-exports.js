...
export function parseInstitutionExportRequest(input) {
  ...
  const requestedSource = clean(searchParams.get("source")).toLowerCase();
  const source = requestedSource === "legacy" ? "legacy" : "neon";
  ...
  const selectedTagIds = parseListParam(searchParams.getAll("tag")).map(clean).filter(Boolean);
  const pdfOrientation = clean(searchParams.get("pdfOrientation")).toLowerCase();
  const pdfTextSize = clean(searchParams.get("pdfTextSize")).toLowerCase();
...
export async function buildInstitutionPdfExport(input) {
  const { renderInstitutionPdf } = await import("./institution-pdf");
  const config = parseInstitutionExportRequest(input);
  const students = await listInstitutionExportStudents(config);
  ...
  const pdf = await renderInstitutionPdf({
    ...
    fontSizePreference: config.pdfTextSize
  });
