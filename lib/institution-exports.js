...
export async function buildInstitutionCsvExport(input) {
  ...
  const filenameScope = config.scopedInstitutionCode || "filtered";
  const filenamePrefix = config.source === "neon" ? "students-neon" : "students";
...
export async function buildInstitutionPdfExport(input) {
  ...
  const filenameScope = config.scopedInstitutionCode || "filtered";
  const filenamePrefix = config.source === "neon" ? "students-neon" : "students";
  const filename = `${filenamePrefix}-${filenameScope}-${new Date().toISOString().slice(0, 10)}.pdf`;
