...
function safeInternalPath(value) {
  ...
  if (raw.startsWith("/sign-in")) return "";
  if (raw.startsWith("/sign-up")) return "";
  return raw;
}

export async function signInRedirectUrl(fallback = "/") {
  const headerList = await headers();
  const currentPath = safeInternalPath(headerList.get("x-current-path"));
  const fallbackPath = safeInternalPath(fallback) || "/";
  const returnPath = currentPath || fallbackPath;
  if (!returnPath || returnPath === "/") return "/sign-in";
  return `/sign-in?redirect_url=${encodeURIComponent(returnPath)}`;
}

const OFFICE_STAFF_LABEL = "משרד";

function normalizeInviteIdPart(value) {
  return clean(value).toLowerCase().replace(/[^a-z0-9_.:-]+/g, "-").replace(/^-+|-+$/g, "");
}

export function buildStudentWhatsAppInviteUserId(studentId) {
  const id = clean(studentId);
  return id ? `student:${id}` : "";
}

export function buildStaffWhatsAppInviteUserId(email) {
  const normalizedEmail = normalizeEmail(email);
  return normalizedEmail ? `staff:${normalizeInviteIdPart(normalizedEmail)}` : "";
}

function inviteEmail(prefix, value) {
  const id = normalizeInviteIdPart(value) || randomUUID();
  return `${prefix}-${id}@whatsapp.local`;
}
...
let appUserAgentColumnsPromise = null;

async function ensureAppUserAgentColumns() {
  if (appUserAgentColumnsPromise) return appUserAgentColumnsPromise;
  appUserAgentColumnsPromise = (async () => {
  ...
  `;
    ...
    appUserAgentColumnsPromise = null;
    throw error;
  });
  return appUserAgentColumnsPromise;
}

function mapAppUserRow(user) {
  if (!user) return null;
  const staffLabels = Array.isArray(user.staff_labels) ? user.staff_labels.map(clean).filter(Boolean) : [];
  ...
  const isPrintOnly = role === "print_only";
  const isMareiMekomot = role === "marei_mekomot";
  return {
    ...user,
    ...
    print_color_enabled: isSuperAdmin || Boolean(user.print_color_enabled),
    can_call_attendance: isSuperAdmin || isManager || isTeamMember || Boolean(user.can_call_attendance),
    staff_labels: staffLabels,
    is_office_staff: staffLabels.includes(OFFICE_STAFF_LABEL),
    ...
    is_print_only: isPrintOnly,
    is_marei_mekomot: isMareiMekomot,
    can_use_print_queue: isTeamMember || isManager || isSuperAdmin || isPrintOnly || isMareiMekomot,
    can_use_announcement_templates: isTeamMember || isManager || isSuperAdmin || isMareiMekomot,
...
async function fetchAppUserByClerkId(clerkUserId) {
  ...
  const rows = await sql`
    SELECT
      ...
      u.print_color_enabled,
      u.can_call_attendance,
...
export async function requireAttendanceUser() {
  const user = await requireAuthenticatedUser();
  if (!user.is_team_member && !user.is_manager && !user.is_super_admin) {
...
export async function ensureStudentWhatsAppInviteUser({
  studentId,
  studentName = "",
  studentClass = "",
  canPrint = false,
  ...
  const inviteUserId = buildStudentWhatsAppInviteUserId(normalizedStudentId);
  const role = canPrint ? "print_only" : "viewer";
  await sql`
    INSERT INTO app_users (
      clerk_user_id,
      email,
    ...
    VALUES (
      ${inviteUserId},
      ${inviteEmail("student", normalizedStudentId)},
      ${clean(studentName) || "תלמיד WhatsApp"},
      ${role},
      'approved',
      ${normalizedStudentId},
      ${clean(studentClass) || null},
      FALSE,
      'whatsapp',
      TRUE,
      FALSE,
      ${clean(approvedByUserId) || null}
    )
    ...
    DO UPDATE SET
      ...
      linked_student_class = EXCLUDED.linked_student_class,
      preferred_agent_channel = 'whatsapp',
      agent_whatsapp_enabled = TRUE,
      agent_telegram_enabled = COALESCE(app_users.agent_telegram_enabled, FALSE),
      approved_by_user_id = COALESCE(app_users.approved_by_user_id, EXCLUDED.approved_by_user_id),
      updated_at = NOW()
  `;
  return fetchAppUserByClerkId(inviteUserId);
}

export async function ensureStaffWhatsAppInviteUser({
  email,
  displayName = "",
  approvedByUserId = ""
} = {}) {
  ...
  await ensureAppUserAgentColumns();
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) throw new Error("כדי ליצור חיבור לאיש צוות צריך להזין מייל.");
  ...
  const existingRows = await sql`
    ...
    LIMIT 1
  `;
  const inviteUserId = clean(existingRows[0]?.clerk_user_id) || buildStaffWhatsAppInviteUserId(normalizedEmail);
  ...
  await sql`
    ...
    VALUES (
      ${inviteUserId},
      ${normalizedEmail},
      ${clean(displayName) || normalizedEmail},
      'editor',
      'approved',
      NULL,
      'TEAM',
    ...
    DO UPDATE SET
      ...
      role = CASE
        WHEN LOWER(COALESCE(app_users.role, '')) = 'super_admin' THEN app_users.role
        ELSE 'editor'
      END,
      access_status = 'approved',
      linked_student_class = 'TEAM',
      can_send_emails = TRUE,
      can_email_parents = TRUE,
      can_view_email_reports = TRUE,
      preferred_agent_channel = 'whatsapp',
...
export async function setAppUserRole(targetClerkUserId, role) {
  ...
  if (!["viewer", "print_only", "marei_mekomot", "editor", "admin", "super_admin"].includes(normalizedRole)) {
...
export async function setUserAgentChannelPreferences(targetClerkUserId, {
  ...
  const shouldUpdateStaffLabels = Array.isArray(staffLabels);
  const normalizedStaffLabels = shouldUpdateStaffLabels ? [...new Set(staffLabels.map(clean).filter(Boolean))] : [];
  await sql`
    UPDATE app_users
    SET
      ...
      staff_labels = CASE WHEN ${shouldUpdateStaffLabels} THEN ${normalizedStaffLabels} ELSE staff_labels END,
      updated_at = NOW()
    WHERE clerk_user_id = ${clean(targetClerkUserId)}
  `;
}

export async function setUserPrintColorPermission(targetClerkUserId, enabled = false) {
  await initDb();
  await ensureAppUserAgentColumns();
  await sql`
    UPDATE app_users
    SET print_color_enabled = ${Boolean(enabled)}, updated_at = NOW()
    WHERE clerk_user_id = ${clean(targetClerkUserId)}
  `;
}

export async function setUserCallAttendancePermission(targetClerkUserId, enabled = false) {
  await initDb();
  await ensureAppUserAgentColumns();
  await sql`
    UPDATE app_users
    SET can_call_attendance = ${Boolean(enabled)}, updated_at = NOW()
    WHERE clerk_user_id = ${clean(targetClerkUserId)}
  `;
}
