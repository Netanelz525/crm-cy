...
function extractDateRangeFromText(text) {
  ...
  const monthWithYearMatch = normalized.match(/\b(ינואר|פברואר|מרץ|מרס|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר)\s+(20\d{2})\b/);
  if (monthWithYearMatch) {
    const monthNumber = HEBREW_MONTHS[monthWithYearMatch[1]];
    const year = Number(monthWithYearMatch[2]);
    return buildMonthRange(year, monthNumber) || defaults;
  }

  const monthStandaloneMatch = normalized.match(/\b(ינואר|פברואר|מרץ|מרס|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר)\b/);
  if (monthStandaloneMatch) {
    const monthNumber = HEBREW_MONTHS[monthStandaloneMatch[1]];
    let year = today.getUTCFullYear();
    if (monthNumber > today.getUTCMonth() + 1) {
      year -= 1;
    }
...
function isPaymentReportIntent(text) {
  ...
  return (hasPaymentNoun && (hasReportVerb || hasDateHint || hasSourceHint || hasMoneyQuestion))
    || (hasSourceHint && (hasReportVerb || hasDateHint || hasMoneyQuestion));
}
