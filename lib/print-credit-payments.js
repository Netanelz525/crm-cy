...
export async function createPrintCreditPurchaseIntent({ user, packageKey, callbackUrl }) {
  await initDb();
  ...
  const { firstName, lastName } = splitName(user?.display_name, email);
  ...
  const transaction = {
    ...
    Street: "",
    City: "",
    Phone: onlyDigits(user?.phone || user?.studentPhone || ""),
