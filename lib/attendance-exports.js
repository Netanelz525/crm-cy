import { ATTENDANCE_STATUS_LABELS, buildAttendanceStatusLabels, getAttendanceRoster } from "./attendance";
...
function buildExportRows(students, sortMode = "class_name", statusLabels = ATTENDANCE_STATUS_LABELS) {
  return [...(students || [])]
    .sort((left, right) => (
      (sortMode === "status"
    ...
    .map((student, index) => ({
      ...
      attendanceStatus: statusLabels[clean(student.status).toLowerCase()] || ATTENDANCE_STATUS_LABELS.missing,
...
export async function buildAttendancePdfExport(sessionId, { sort = "class_name" } = {}) {
  ...
  const rows = buildExportRows(roster.students, sortMode, statusLabels);
  const title = [
    "דוח נוכחות",
    clean(roster.session.institutionLabel),
    clean(roster.session.displayTitle || roster.session.title || roster.session.sessionTypeLabel)
  ].filter(Boolean).join(" - ");
  ...
  const baseName = [
    ...
    fileNamePart(roster.session.displayTitle || roster.session.title || roster.session.sessionTypeLabel),
