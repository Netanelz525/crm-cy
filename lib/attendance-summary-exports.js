...
function buildSummaryRows(report) {
  return (report?.rows || []).map((student) => {
    const row = {
...
export async function buildAttendanceSummaryExcelExport(input) {
  const report = await requireSummaryReport(input);
...
export async function buildAttendanceSummaryPdfExport(input) {
  const report = await requireSummaryReport(input);
  const rows = buildSummaryRows(report).map((row, index) => ({
  ...
  const pdf = await renderInstitutionPdf({
    ...
    subtitle: `טווח: ${clean(report.dateFrom)} עד ${clean(report.dateTo)} | תלמידים: ${report.totalStudents} | מפגשים: ${report.totalSessions}`,
