...
function hasPhone(value) {
  ...
  return numbers.some((number) => clean(number).replace(/\D/g, "").length >= 7);
}
...
function requireCaller(user) {
  ...
  if (!user.can_call_attendance) throw new CallQueueError("המשתמש אינו מורשה להתקשר במוקד המפגש.", 403);
}
...
export async function getCallTeam(sessionId, user) {
  ...
  const attendance = new Map((roster?.students || []).map(s => [s.id, {status:s.status,statusLabel:roster.session.statusOptions.find(([v])=>v===s.status)?.[1]||s.status}]));
  return { selected: teams[0]?.student_ids || [], students: students.map(s => ({id:s.id, label:s.label || s.name || s.id,...attendance.get(s.id)})), leads, attempts, statusOptions:roster?.session?.statusOptions || [] };
}

export async function manageAttendanceCall(sessionId,input,user){
  requireManager(user); await ensureAttendanceCalls();
  if(input.kind==='requeue'){await sql`UPDATE attendance_call_leads SET outcome='pending',available_at=NOW(),caller_key=NULL,lease_token=NULL,lease_until=NULL,updated_at=NOW() WHERE session_id=${sessionId} AND student_id=${clean(input.studentId)}`;return {ok:true};}
  const outcome=clean(input.outcome),note=clean(input.note),status=clean(input.attendanceStatus);
  if(!Object.hasOwn(CALL_OUTCOMES,outcome)||note.length>4000)throw new CallQueueError('פרטי העדכון אינם תקינים.');
  const roster=await getAttendanceRoster(sessionId); if(status&&!roster.session.statusOptions.some(([v])=>v===status))throw new CallQueueError('סטטוס ההגעה אינו תקין.');
  const rows=await sql`UPDATE attendance_call_attempts SET outcome=${outcome},note=${note},attendance_status=${status||null} WHERE session_id=${sessionId} AND token=${clean(input.token)} RETURNING student_id`;
  if(!rows.length)throw new CallQueueError('תיעוד השיחה לא נמצא.',404);
  await sql`UPDATE attendance_call_leads SET outcome=${outcome},updated_at=NOW() WHERE session_id=${sessionId} AND student_id=${rows[0].student_id}`;
  const student=roster.students.find(s=>s.id===rows[0].student_id);
  if(status&&student)await sql`INSERT INTO attendance_records(session_id,student_id,student_name,student_class,status,note_text,marked_by_user_id,marked_at) VALUES(${sessionId},${student.id},${student.label},${student.class},${status},${note},${user.clerk_user_id},NOW()) ON CONFLICT(session_id,student_id) DO UPDATE SET status=EXCLUDED.status,note_text=EXCLUDED.note_text,marked_by_user_id=EXCLUDED.marked_by_user_id,marked_at=NOW(),updated_at=NOW()`;
  return {ok:true};
}
...
async function accessRoster(sessionId, user) {
  ...
  return {
    ...roster,
    // Students without a student/parent phone cannot be handled by a phone desk.
    // Call outcomes remain separate from completion: only `completed` is counted
    // as handled; `deferred` and `no_answer` are retry states.
    students: roster.students.filter((student) => (
      clean(student.status).toLowerCase() !== "unavailable" && hasCallContact(student)
    ))
  };
}
  ...
  try {
    return await sql.transaction([
      ...
      sql`SELECT 1 / COUNT(*)::int AS allowed FROM attendance_sessions s
        ...
        AND (COALESCE(u.can_call_attendance,FALSE) OR LOWER(COALESCE(u.role,'')) IN ('admin','editor','super_admin')
          OR UPPER(COALESCE(u.linked_student_class,''))='TEAM')
...
export async function claimAttendanceCall(sessionId, user, requestedStudentId = "") {
  ...
  return { session: { id:sessionId, title:roster.session.displayTitle || roster.session.title || roster.session.sessionTypeLabel,
    ...
    lead:student ? { studentId:student.id, name:student.label, classLabel:student.classLabel,
      ...
      father:{name:student.fatherName,phone:student.dadPhone,email:student.fatherEmail},
      mother:{name:student.motherName,phone:student.momPhone,email:student.motherEmail},
      status:student.status, token:lead.lease_token, expiresAt:lead.lease_until, history } : null };
}
