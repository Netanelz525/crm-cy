...
export function getTelegramBotUsername() {
  return clean(process.env.BOT_TELEGRAM_USERNAME || "crmcy_bot").replace(/^@/, "");
}
...
export function buildTelegramDeepLink(code) {
  const username = getTelegramBotUsername();
  const normalizedCode = clean(code);
  if (!username || !normalizedCode) return "";
  return `https://t.me/${username}?start=${encodeURIComponent(normalizedCode)}`;
}
...
export async function sendTelegramDocument(chatId, document, options = {}) {
  const payload = {
    chat_id: clean(chatId),
  ...
  const response = await fetch(`${getTelegramApiBase()}/sendDocument`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload)
  });
...
export async function sendTelegramDocumentFile(chatId, file, options = {}) {
  const form = new FormData();
  form.set("chat_id", clean(chatId));
  form.set("document", new Blob([file.content], { type: file.contentType || "application/octet-stream" }), clean(file.filename) || "document");
  if (clean(options.caption)) form.set("caption", clean(options.caption).slice(0, 1024));
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok || !data?.ok) {
    throw new Error(data?.description || "Telegram sendDocument file upload failed");
  }
  return data.result;
}

export async function editTelegramMessageReplyMarkup({ chatId, messageId, replyMarkup }) {
  const response = await fetch(`${getTelegramApiBase()}/editMessageReplyMarkup`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      chat_id: clean(chatId),
  ...
  const data = await response.json().catch(() => null);
  if (!response.ok || !data?.ok) {
    throw new Error(data?.description || "Telegram editMessageReplyMarkup failed");
  }
  return data.result;
}
