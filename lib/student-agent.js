...
function normalizeEnumValue(enumName, rawValue) {
  ...
  for (const [value, label] of Object.entries(options)) {
    if (normalizeText(value) === rawNormalized || normalizeText(label) === rawNormalized) {
      return value;
    }
  }
...
export function inferEnumFiltersFromQuery(query) {
  ...
  const inferred = [];
  const enumFields = [
    { enumName: "currentInstitution", field: "currentInstitution" },
    { enumName: "class", field: "class" },
    { enumName: "registration", field: "registration" },
    { enumName: "familystatus", field: "famliystatus" }
  ];

  for (const enumField of enumFields) {
    for (const [value, label] of Object.entries(ENUM_LABELS[enumField.enumName] || {})) {
      const valueText = normalizeText(value);
      const labelText = normalizeText(label);
      const queryText = normalizeText(raw);
      if (!queryText) continue;
      if (queryText === valueText || queryText === labelText || queryText.includes(labelText) || queryText.includes(valueText)) {
        inferred.push({
...
function filterStudent(student, rawFilter) {
  ...
  if (Array.isArray(actual)) {
    return actual.some((item) => compareValues(item, operator, rawFilter?.value));
  }

  if (fieldDef.enum) {
    const normalizedExpected = normalizeEnumValue(fieldDef.enum, rawFilter?.value);
    if (compareValues(actual, operator, normalizedExpected)) return true;
    return compareValues(enumLabel(fieldDef.enum, actual), operator, rawFilter?.value);
  }

  return compareValues(actual, operator, rawFilter?.value);
}
...
export async function findStudentsForAgent({ query = "", filters = [], minScore = 0.3 } = {}) {
  ...
  if (effectiveFilters.length) {
    ...
    students = await searchNeonStudents({ q: query, minScore });
  } else {
    students = await listAllNeonStudents();
  }
...
export async function searchStudentsForAgent({ query = "", filters = [], limit = 10, minScore = 0.3 } = {}) {
