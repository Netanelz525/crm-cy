...
export function inferEnumFiltersFromQuery(query) {
  ...
  for (const enumField of enumFields) {
    ...
    for (const [value, label] of entries) {
      ...
      const valueMentioned = valueText.length >= 2 && new RegExp(`(^|\\s)${valueText.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}($|\\s)`).test(queryText);
      const labelMentioned = labelText.length >= 2 && (queryText === labelText || queryText.includes(labelText));
      if (queryText === valueText || valueMentioned || labelMentioned) {
        inferred.push({
