...
function estimateRowsPerPage(columns, orientation) {
  ...
  if (columns.length <= 4) return isLandscape ? 45 : 48;
  if (columns.length <= 6) return isLandscape ? 37 : 40;
  if (isLandscape) return columns.length > 10 ? 25 : 29;
  if (columns.length > 9) return 31;
  if (columns.length > 7) return 35;
  return 41;
}
...
function buildHtml({ title, subtitle, columns, rows, exportedAt, orientation, fontUrl }) {
  ...
  const fontSize = isLandscape
    ...
    : (columns.length > 8 ? 8.5 : columns.length > 6 ? 9 : 10);
