...
function estimateRowsPerPage(columns, orientation) {
  ...
  if (isLandscape) return columns.length > 10 ? 24 : 27;
  if (columns.length > 9) return 28;
  if (columns.length > 7) return 32;
  return 38;
}

function estimatePageSizes(columns, orientation) {
  const basePageSize = estimateRowsPerPage(columns, orientation);
  const firstPageSize = Math.max(18, basePageSize + 1);
  const nextPageSize = basePageSize + 1;
  return {
    firstPageSize,
    nextPageSize
  };
...
function buildHtml({ title, subtitle, columns, rows, exportedAt, orientation, fontUrl }) {
  ...
  const fontSize = isLandscape
    ? (columns.length > 9 ? 7.5 : 8.5)
    : (columns.length > 8 ? 7 : columns.length > 6 ? 7.5 : 8.5);
