...
function estimateRowsPerPage(columns, orientation) {
  ...
  if (columns.length <= 4) return isLandscape ? 38 : 40;
  if (columns.length <= 6) return isLandscape ? 32 : 34;
  if (isLandscape) return columns.length > 10 ? 23 : 28;
  if (columns.length > 9) return 28;
  if (columns.length > 7) return 30;
  return 34;
}
...
function chunkRows(rows, firstPageSize, nextPageSize) {
  ...
  while (index < rows.length) {
    const pageSize = pages.length === 0 ? firstPageSize : nextPageSize;
