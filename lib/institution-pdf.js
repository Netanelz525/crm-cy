...
function estimateRowsPerPage(columns, orientation) {
  ...
  if (isLandscape) return columns.length > 10 ? 18 : 20;
  if (columns.length > 9) return 20;
  if (columns.length > 7) return 22;
  return 26;
}

function estimatePageSizes(columns, orientation) {
  const basePageSize = estimateRowsPerPage(columns, orientation);
  const firstPageSize = Math.max(10, basePageSize - 3);
  const nextPageSize = basePageSize + 8;
  return {
    firstPageSize,
    nextPageSize
  };
