...
function estimateRowsPerPage(columns, orientation) {
  ...
  if (columns.length <= 4) return isLandscape ? 33 : 35;
  if (columns.length <= 6) return isLandscape ? 28 : 30;
  if (isLandscape) return columns.length > 10 ? 19 : 24;
  if (columns.length > 9) return 25;
  if (columns.length > 7) return 27;
  return 31;
}
...
function chunkRows(rows, firstPageSize, nextPageSize) {
  ...
  for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
    ...
    const maxSizeAllowed = Math.min(pageCapacity, remainingRows - minForRest);
    const size = remainingPages === 1 ? remainingRows : Math.max(1, maxSizeAllowed);
