...
function estimateRowUnits(row, columns) {
  const textLength = columns.reduce((sum, column) => {
    return sum + clean(row?.[column.key]).length;
  }, 0);
  if (textLength > 240) return 2.2;
  if (textLength > 160) return 1.8;
  if (textLength > 110) return 1.45;
  return 1;
}

function paginateRows(rows, columns, orientation) {
  const maxUnitsPerPage = orientation === "landscape" ? 26 : 32;
  const pages = [];
  let currentPage = [];
  let currentUnits = 0;

  rows.forEach((row) => {
    const rowUnits = estimateRowUnits(row, columns);
    if (currentPage.length && currentUnits + rowUnits > maxUnitsPerPage) {
...
function buildHtml({ title, subtitle, columns, rows, exportedAt, orientation, fontUrl }) {
  ...
  const pages = paginateRows(rows, columns, orientation);
      ...
      ${rows.length
        ? pages.map((pageRows) => `
            <section class="report-page">
              ...
              <div class="report-footer">סוף עמוד</div>
            </section>
          `).join("")
