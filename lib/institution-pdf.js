...
const PDF_TEXT_SIZE_PRESETS = {
  large: { fontSize: 15.5, lineHeight: 1.35, cellPaddingY: 5, headerFontSize: 12.5, maxRowsPerPage: 31 },
  medium: { fontSize: 14.5, lineHeight: 1.3, cellPaddingY: 4, headerFontSize: 12, maxRowsPerPage: 34 },
  small: { fontSize: 13, lineHeight: 1.2, cellPaddingY: 3, headerFontSize: 11, maxRowsPerPage: 40 }
};

function normalizePdfTextSize(value) {
  const normalized = clean(value).toLowerCase();
  return Object.hasOwn(PDF_TEXT_SIZE_PRESETS, normalized) ? normalized : "large";
}

function choosePdfTextLayout(rowCount, orientation, requestedSize) {
  const preferred = normalizePdfTextSize(requestedSize);
  const candidates = preferred === "large"
    ? ["large", "medium", "small"]
    : preferred === "medium"
      ? ["medium", "small"]
      : ["small"];
  const landscapeFactor = orientation === "landscape" ? 0.68 : 1;

  for (const key of candidates) {
    const preset = PDF_TEXT_SIZE_PRESETS[key];
    const maxRowsPerPage = Math.max(1, Math.floor(preset.maxRowsPerPage * landscapeFactor));
    const pageCount = Math.max(1, Math.ceil(rowCount / maxRowsPerPage));
    if (pageCount <= 2 || key === candidates[candidates.length - 1]) {
      return { key, preset, maxRowsPerPage, pageCount };
    }
  }

  return {
    key: "small",
    preset: PDF_TEXT_SIZE_PRESETS.small,
    maxRowsPerPage: Math.max(1, Math.floor(PDF_TEXT_SIZE_PRESETS.small.maxRowsPerPage * landscapeFactor)),
    pageCount: Math.max(1, Math.ceil(rowCount / PDF_TEXT_SIZE_PRESETS.small.maxRowsPerPage))
  };
}

function chunkRows(rows, maxRowsPerPage) {
  if (!rows.length) return [];

  // Balance the pages instead of leaving a short overflow page at the end.
  const pageCount = Math.ceil(rows.length / maxRowsPerPage);
  const rowsPerPage = Math.ceil(rows.length / pageCount);
  const chunks = [];
...
function buildHtml({ title, subtitle, columns, rows, exportedAt, orientation, fontUrl, textLayout }) {
  const isLandscape = orientation === "landscape";
  const { preset, maxRowsPerPage } = textLayout;
  const fontSize = isLandscape ? Math.max(11, preset.fontSize - 1.5) : preset.fontSize;
  ...
  const rowChunks = chunkRows(rows, maxRowsPerPage);
  ...
  <head>
    ...
    <style>
      ...
      th, td {
        ...
        padding: ${preset.cellPaddingY}px 6px;
      ...
      th {
        ...
        color: #0f172a;
        font-weight: 700;
        font-size: ${preset.headerFontSize}px;
        padding-top: ${preset.cellPaddingY + 1}px;
        padding-bottom: ${preset.cellPaddingY + 1}px;
      }
      .row-number-head,
      .row-number-cell {
        width: 8mm;
...
export async function renderInstitutionPdf({ title, subtitle, columns, rows, orientation, fontSizePreference = "large" }) {
  const exportedAt = new Date().toISOString();
  const fontUrl = await getPdfFontDataUrl();
  const effectiveOrientation = inferOrientation(columns, orientation);
  const textLayout = choosePdfTextLayout(rows.length, effectiveOrientation, fontSizePreference);
