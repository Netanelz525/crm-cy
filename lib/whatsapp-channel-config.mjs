...
export function coexistenceWhatsAppConfig(env = process.env) {
  const baseUrl = (clean(env.WHATSAPP_COEX_API_BASE) || "https://api.dualhook.com").replace(/\/+$/, "");
  const version = clean(env.WHATSAPP_COEX_API_VERSION) || "v25.0";
  const phoneNumberId = required(env.WHATSAPP_COEX_PHONE_NUMBER_ID, "WHATSAPP_COEX_PHONE_NUMBER_ID");
  const wabaId = required(env.WHATSAPP_COEX_WABA_ID, "WHATSAPP_COEX_WABA_ID");
  return {
    baseUrl,
    version,
    phoneNumberId,
    wabaId,
    accessToken: required(env.WHATSAPP_COEX_ACCESS_TOKEN, "WHATSAPP_COEX_ACCESS_TOKEN"),
...
export function isCoexistenceWhatsAppConfigured(env = process.env) {
  return Boolean(
    clean(env.WHATSAPP_COEX_ACCESS_TOKEN)
    && clean(env.WHATSAPP_COEX_PHONE_NUMBER_ID)
    && clean(env.WHATSAPP_COEX_WABA_ID)
  );
}
