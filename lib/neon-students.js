...
export async function searchNeonStudents({
  ...
  const out = students.filter((student) => {
    ...
    if (normalizedClass && clean(student?.class).toUpperCase() !== normalizedClass) {
      return false;
    }

    if (normalizedRegistration && clean(student?.registration).toUpperCase() !== normalizedRegistration) {
      return false;
    }

    if (normalizedFamilyStatus && clean(student?.famliystatus).toUpperCase() !== normalizedFamilyStatus) {
      return false;
    }
