...
function buildNeonQuery({
  ...
  const normalizedStudentIds = (Array.isArray(studentIds) ? studentIds : [studentIds]).map(clean).filter(Boolean);
  ...
  let query = `${BASE_NEON_SELECT}
    WHERE ($1 = '' OR current_institution = ANY(string_to_array($1, ',')))
      ...
      AND ($9 = '' OR (
        student_id = ANY(string_to_array($9, ','))
        OR COALESCE(payload->>'id', '') = ANY(string_to_array($9, ','))
      ))
...
export async function buildStudentMirrorRecord(student) {
...
export async function searchNeonStudentsByText(q, maxResults = 10, minScore = 0.42) {
  ...
  return maxResults === null ? matches : matches.slice(0, Math.max(0, Number(maxResults) || 0));
}
