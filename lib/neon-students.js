...
import { createStudentByData, deleteStudentById, getStudentById, getStudentByPrimaryEmail, listAllStudents, searchStudentsByTz, updateStudentById } from "./twenty";
  ...
  let query = `${BASE_NEON_SELECT}
    WHERE ($1 = '' OR current_institution = $1)
      ...
      AND ($6 = '' OR (
        ...
        OR COALESCE(payload->'adders'->>'addressStreet1', '') ILIKE $6
        OR COALESCE(payload->'adders'->>'addressStreet2', '') ILIKE $6
        OR COALESCE(payload->'adders'->>'addressCity', '') ILIKE $6
        OR COALESCE(payload->'adders'->>'addressPostcode', '') ILIKE $6
        OR COALESCE(payload->'adders'->>'addressState', '') ILIKE $6
        OR COALESCE(payload->'adders'->>'addressCountry', '') ILIKE $6
        OR COALESCE(payload->>'shmHb', '') ILIKE $6
        OR COALESCE(payload->>'shmHm', '') ILIKE $6
      ))
      AND ($7 = '' OR (
        tznum ILIKE $7
...
export async function createNeonStudentViaTwenty(data) {
  ...
  const duplicate = await findDuplicateStudentForCreate(twentyData);
  if (duplicate?.student?.id) {
    const existingStudent = await syncNeonStudentFromTwentyById(duplicate.student.id).catch(() => null);
    ...
    error.student = existingStudent || duplicate.student;
    throw error;
  }
