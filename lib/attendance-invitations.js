...
export async function sendAttendanceInvitation({ sessionId, studentId = "", channels = [] } = {}) {
  ...
  if (selectedChannels.includes("email")) {
    const invitationMessage = clean(session.invitationMessage || session.invitationEmailBody);
    if (!invitationMessage) throw new Error("לא הוגדר תוכן להודעת ההזמנה.");
    result.email = await sendAttendanceInvitationEmails({
      sessionId,
      studentId,
      subject: session.invitationEmailSubject || session.displayTitle || session.title,
