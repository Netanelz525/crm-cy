...
export async function sendAttendanceInvitation({ sessionId, studentId = "", channels = [] } = {}) {
  ...
  if (selectedChannels.includes("whatsapp")) {
    ...
    if (!template) throw new Error("תבנית WhatsApp להזמנה אינה מאושרת או שאינה זמינה בחיבור של בוט התפוצה.");
    if (template.requiresMedia && !attachment) {
      const requiredFile = template.headerFormat === "DOCUMENT" ? "קובץ PDF" : "תמונה";
      throw new Error(`התבנית שנבחרה דורשת ${requiredFile} בכותרת. יש לצרף אותו בהגדרות ההזמנה או לבחור תבנית ללא ${requiredFile}.`);
    }
    whatsappContext = { templates, template };
  }
