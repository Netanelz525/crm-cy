...
export const ATTENDANCE_COMMUNICATION_AUDIENCE_LABELS = {
  student: "התלמידים",
  parents: "ההורים בנוגע לתלמיד",
  parent_meeting: "פגישה עם ההורים"
};
...
export function parseAttendanceCustomStatusesText(value) {
  return String(value || "")
    ...
    .map((line) => {
      ...
      const [apiName, ...labelParts] = line.split(separator);
      return {
        value: clean(apiName),
        label: clean(labelParts.join(separator))
      };
    });
}

export function parseAttendanceCustomStatusesFields(apiValues, labelValues) {
  const apiNames = Array.isArray(apiValues) ? apiValues : [apiValues];
  const labels = Array.isArray(labelValues) ? labelValues : [labelValues];
  const rowCount = Math.max(apiNames.length, labels.length);
  return Array.from({ length: rowCount }, (_, index) => ({
    value: clean(apiNames[index]),
    label: clean(labels[index])
...
function normalizeCommunicationAudience(value) {
  const normalized = clean(value).toLowerCase();
  return ["parents", "parent_meeting"].includes(normalized) ? normalized : "student";
}
...
function mapSessionFilters(row) {
  ...
  const manualStudentIds = unique((Array.isArray(row?.manual_student_ids) ? row.manual_student_ids : [row?.manual_student_ids]).map(clean).filter(Boolean));
  const excludedStudentIds = unique((Array.isArray(row?.excluded_student_ids) ? row.excluded_student_ids : [row?.excluded_student_ids]).map(clean).filter(Boolean));
  return {
    institutionFilter,
...
function mapSessionRow(row) {
  ...
  const invitationEmailRecipientRoles = normalizeAttendanceEmailRecipientRoles(row.invitation_email_recipient_roles);
  const invitationWhatsAppRecipientRoles = unique((Array.isArray(row.invitation_whatsapp_recipient_roles) ? row.invitation_whatsapp_recipient_roles : []).map(clean).filter((value) => ["student", "father", "mother"].includes(value)));
  ...
  const hasAudienceFilters = Boolean(
    filters.institutionFilter.length
    || filters.classFilter.length
    || filters.registrationFilter.length
    || filters.familyStatusFilter.length
    || filters.tagFilter.length
    || (institution && institution !== "ALL")
  );
  const storedRosterMode = clean(row.roster_mode);
  // A manually curated roster is authoritative when no audience filters exist.
  // This also repairs older rows that were saved before roster_mode was added.
  const rosterMode = !hasAudienceFilters && filters.manualStudentIds.length
    ? "manual"
    : storedRosterMode || (hasAudienceFilters ? "filters" : "manual");
  ...
  return {
    ...
    communicationAudience: normalizeCommunicationAudience(row.communication_audience),
    ...
    invitationEmailSubject: clean(row.invitation_email_subject),
    invitationEmailBody: clean(row.invitation_email_body),
    invitationMessage: clean(row.invitation_message) || clean(row.invitation_email_body) || clean(row.source_note),
    invitationEmailRecipientRoles: invitationEmailRecipientRoles.length ? invitationEmailRecipientRoles : DEFAULT_ATTENDANCE_EMAIL_RECIPIENT_ROLES,
    invitationWhatsAppTemplateName: clean(row.invitation_whatsapp_template_name),
    invitationWhatsAppTemplateLanguage: clean(row.invitation_whatsapp_template_language) || "he",
    invitationWhatsAppRecipientRoles: invitationWhatsAppRecipientRoles.length ? invitationWhatsAppRecipientRoles : ["student"],
    invitationAttachment: clean(row.invitation_attachment_object_key) ? {
      objectKey: clean(row.invitation_attachment_object_key),
      fileName: clean(row.invitation_attachment_file_name),
      contentType: clean(row.invitation_attachment_content_type),
      sizeBytes: Number(row.invitation_attachment_size_bytes) || 0
    } : null,
    ...
    manualStudentIds: filters.manualStudentIds,
    rosterMode,
    excludedStudentIds: filters.excludedStudentIds,
    visibleToStudents: row.visible_to_students === true,
    responsibleUserIds: unique((Array.isArray(row.responsible_user_ids) ? row.responsible_user_ids : [row.responsible_user_id]).map(clean).filter(Boolean)),
    responsibleUsers: Array.isArray(row.responsible_user_options) ? row.responsible_user_options : [],
    responsibleUserId: clean(row.responsible_user_id) || clean((Array.isArray(row.responsible_user_ids) ? row.responsible_user_ids : [])[0]),
    responsibleDisplayName: (Array.isArray(row.responsible_user_options) ? row.responsible_user_options : [])
      .map((user) => clean(user.label))
      .filter(Boolean)
      .join(", "),
    responsibleEmail: "",
...
function mapRecordRow(row, statusLabels = ATTENDANCE_STATUS_LABELS) {
  ...
  return {
    ...
    firstResponseText: clean(row.first_response_text),
    firstResponseAt: row.first_response_at || null,
    firstResponseChannel: clean(row.first_response_channel),
...
export async function createAttendanceSession({
  ...
  invitationEmailBody = "",
  invitationMessage = "",
  invitationEmailRecipientRoles = DEFAULT_ATTENDANCE_EMAIL_RECIPIENT_ROLES,
  invitationWhatsAppTemplateName = "",
  invitationWhatsAppTemplateLanguage = "he",
  invitationWhatsAppRecipientRoles = ["student"],
  invitationAttachmentObjectKey = "",
  invitationAttachmentFileName = "",
  invitationAttachmentContentType = "",
  invitationAttachmentSizeBytes = 0,
  customStatuses = [],
  institutionFilter = [],
  classFilter = [],
  registrationFilter = [],
  ...
  const normalizedInvitationEmailRecipientRoles = normalizeAttendanceEmailRecipientRoles(invitationEmailRecipientRoles);
  const storedTitle = clean(title) || ATTENDANCE_SESSION_TYPE_LABELS[normalizedSessionType];
  const storedInvitationMessage = clean(invitationMessage) || clean(invitationEmailBody) || clean(sourceNote);
  const storedInvitationEmailSubject = clean(invitationEmailSubject) || storedTitle;
  const storedInvitationEmailBody = clean(invitationEmailBody) || storedInvitationMessage;
  const normalizedInvitationWhatsAppRecipientRoles = unique((Array.isArray(invitationWhatsAppRecipientRoles) ? invitationWhatsAppRecipientRoles : [invitationWhatsAppRecipientRoles]).map(clean).filter((value) => ["student", "father", "mother"].includes(value)));
  ...
  const normalizedManualStudentIds = unique((Array.isArray(manualStudentIds) ? manualStudentIds : [manualStudentIds]).map(clean).filter(Boolean));
  const normalizedExcludedStudentIds = unique((Array.isArray(excludedStudentIds) ? excludedStudentIds : [excludedStudentIds]).map(clean).filter(Boolean));
  const normalizedRosterMode = ["filters", "manual", "filters_editable"].includes(clean(rosterMode))
    ? clean(rosterMode)
    : (normalizedInstitutionFilter.length || normalizedClassFilter.length ? "filters" : "manual");
  const normalizedResponsibleUserIds = unique([
    ...(Array.isArray(responsibleUserIds) ? responsibleUserIds : [responsibleUserIds]),
    responsibleUserId
  ].map(clean).filter(Boolean));
  const normalizedResponsibleUserId = normalizedResponsibleUserIds[0] || null;
  ...
  await sql`
    INSERT INTO attendance_sessions (
      ...
      invitation_message,
      invitation_email_recipient_roles,
      invitation_whatsapp_template_name,
      invitation_whatsapp_template_language,
      invitation_whatsapp_recipient_roles,
      invitation_attachment_object_key,
      invitation_attachment_file_name,
      invitation_attachment_content_type,
      invitation_attachment_size_bytes,
      institution_filter,
      class_filter,
      registration_filter,
    ...
    VALUES (
      ...
      ${normalizedDate},
      ${clean(sourceNote)},
      ${normalizeCommunicationAudience(communicationAudience)},
      ${clean(emailSubject)},
      ...
      ${storedInvitationEmailSubject},
      ${storedInvitationEmailBody},
      ${storedInvitationMessage},
      ${normalizeAttendanceEmailRecipientRoles(invitationEmailRecipientRoles)},
      ${clean(invitationWhatsAppTemplateName)},
      ${clean(invitationWhatsAppTemplateLanguage) || "he"},
      ${unique((Array.isArray(invitationWhatsAppRecipientRoles) ? invitationWhatsAppRecipientRoles : []).map(clean).filter((value) => ["student", "father", "mother"].includes(value)))},
      ${clean(invitationAttachmentObjectKey) || null},
      ${clean(invitationAttachmentFileName) || null},
      ${clean(invitationAttachmentContentType) || null},
      ${Number(invitationAttachmentSizeBytes) || null},
      ${normalizedInstitutionFilter},
      ${normalizedClassFilter},
      ${normalizedRegistrationFilter},
      ${normalizedFamilyStatusFilter},
      ${normalizedTagFilter},
      ${normalizedManualStudentIds},
      ${normalizedRosterMode},
      ${normalizedExcludedStudentIds},
      ${normalizedResponsibleUserId},
      ${normalizedResponsibleUserIds},
...
export async function getAttendanceSessionById(sessionId) {
  ...
  const rows = await sql`
    SELECT
      ...
      s.session_date,
      s.source_note,
      s.communication_audience,
      s.email_subject,
      s.personal_message,
      s.custom_statuses,
      s.email_response_statuses,
      s.email_recipient_roles,
      s.invitation_email_subject,
      s.invitation_email_body,
      s.invitation_email_recipient_roles,
      s.invitation_whatsapp_template_name,
      s.invitation_whatsapp_template_language,
      s.invitation_whatsapp_recipient_roles,
      s.invitation_attachment_object_key,
      s.invitation_attachment_file_name,
      s.invitation_attachment_content_type,
      s.invitation_attachment_size_bytes,
      s.institution_filter,
      s.class_filter,
      s.registration_filter,
      s.family_status_filter,
      s.tag_filter,
      s.manual_student_ids,
      s.roster_mode,
      s.excluded_student_ids,
      s.responsible_user_id,
      s.responsible_user_ids,
      ...
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object('value', au.clerk_user_id, 'label', COALESCE(NULLIF(au.display_name, ''), au.email, au.clerk_user_id), 'email', au.email) ORDER BY au.display_name, au.email)
        FROM app_users au
        WHERE au.clerk_user_id = ANY(COALESCE(s.responsible_user_ids, ARRAY[]::TEXT[]))
           OR (COALESCE(array_length(s.responsible_user_ids, 1), 0) = 0 AND au.clerk_user_id = s.responsible_user_id)
      ), '[]'::jsonb) AS responsible_user_options,
      COALESCE((
...
export async function listAttendanceSessions({
  ...
  const normalizedResponsibleUserIds = unique((Array.isArray(responsibleUserIds) ? responsibleUserIds : [responsibleUserIds]).map(clean).filter(Boolean));
  const normalizedQuery = clean(query);
  const normalizedInstitutionFilters = normalizeInstitutionFilters(institutionFilters);
  const normalizedClassFilters = normalizeClassFilters(classFilters);
  const normalizedRegistrationFilters = normalizeRegistrationFilters(registrationFilters);
  const normalizedFamilyStatusFilters = normalizeFamilyStatusFilters(familyStatusFilters);
  const normalizedTagFilters = unique((Array.isArray(tagFilters) ? tagFilters : [tagFilters]).map(clean).filter(Boolean));
  ...
  const rows = await sql(
    `
      ...
      WHERE (
        ...
        AND (
          ...
          OR COALESCE(s.responsible_user_ids, ARRAY[]::TEXT[]) && $4::TEXT[]
          OR s.responsible_user_id = ANY($4::TEXT[])
        )
        AND (
          $5 = ''
          OR s.title ILIKE '%' || $5 || '%'
          OR s.source_note ILIKE '%' || $5 || '%'
          OR s.id::TEXT ILIKE '%' || $5 || '%'
          OR s.session_type ILIKE '%' || $5 || '%'
        )
        AND (
          cardinality($6::TEXT[]) = 0
          OR s.institution = ANY($6::TEXT[])
          OR COALESCE(s.institution_filter, ARRAY[]::TEXT[]) && $6::TEXT[]
        )
        AND (
          cardinality($7::TEXT[]) = 0
          OR COALESCE(s.class_filter, ARRAY[]::TEXT[]) && $7::TEXT[]
        )
        AND (
          cardinality($8::TEXT[]) = 0
          OR COALESCE(s.registration_filter, ARRAY[]::TEXT[]) && $8::TEXT[]
        )
        AND (
          cardinality($9::TEXT[]) = 0
          OR COALESCE(s.family_status_filter, ARRAY[]::TEXT[]) && $9::TEXT[]
        )
        AND (
          cardinality($10::TEXT[]) = 0
          OR COALESCE(s.tag_filter, ARRAY[]::TEXT[]) && $10::TEXT[]
        )
    ...
    [
      ...
      normalizedResponsibleUserIds,
      normalizedQuery,
      normalizedInstitutionFilters,
      normalizedClassFilters,
      normalizedRegistrationFilters,
      normalizedFamilyStatusFilters,
...
export async function listAttendanceResponsibleUsers() {
  await initDb();
  const rows = await sql`
    SELECT clerk_user_id, display_name, email, role, linked_student_class, linked_student_id, can_call_attendance
    FROM app_users
    WHERE access_status = 'approved'
      AND (
        LOWER(COALESCE(role, '')) IN ('editor', 'admin', 'super_admin')
        OR UPPER(COALESCE(linked_student_class, '')) = 'TEAM'
        OR NULLIF(BTRIM(COALESCE(linked_student_id, '')), '') IS NOT NULL
        OR COALESCE(can_call_attendance, FALSE) = TRUE
      )
  ...
  return rows.map((row) => ({
    ...
    canCallAttendance: ["editor", "admin", "super_admin"].includes(clean(row.role).toLowerCase())
      || clean(row.linked_student_class).toUpperCase() === "TEAM"
      || Boolean(row.can_call_attendance)
  })).filter((user) => user.id);
}
...
export async function listStudentsForAttendanceInstitution(input) {
  ...
  const institutionFilter = typeof input === "object"
    ...
    : [];
  const manualStudentIds = typeof input === "object"
    ? unique((Array.isArray(input?.manualStudentIds) ? input.manualStudentIds : [input?.manualStudentIds]).map(clean).filter(Boolean))
    : [];
  const rosterMode = typeof input === "object" && ["filters", "manual", "filters_editable"].includes(clean(input?.rosterMode))
    ? clean(input.rosterMode)
    : (institutionFilter.length || institutionCode ? "filters" : "manual");
  const excludedStudentIds = typeof input === "object"
    ? unique((Array.isArray(input?.excludedStudentIds) ? input.excludedStudentIds : [input?.excludedStudentIds]).map(clean).filter(Boolean))
    : [];
  if (rosterMode === "manual" && !manualStudentIds.length) return [];
  if (rosterMode !== "manual" && !institutionCode && !institutionFilter.length && !manualStudentIds.length) return [];
  ...
  const tagFilter = typeof input === "object"
    ...
    : [];
  const hasAudienceFilter = rosterMode !== "manual" && Boolean(
    institutionFilter.length
    || (institutionCode && institutionCode !== "ALL")
    || classFilter.length
    || registrationFilter.length
    || familyStatusFilter.length
    || tagFilter.length
  );
  let students = rosterMode === "manual"
    ? await listNeonStudentsByFilters({ studentIds: manualStudentIds })
    : (hasAudienceFilter
      ? await listNeonStudentsByFilters({
      ...
      : []);
  if (tagFilter.length && students.length) {
    const rows = await sql`
  ...
  if (manualStudentIds.length && rosterMode !== "manual") {
    const manualStudents = await listNeonStudentsByFilters({ studentIds: manualStudentIds });
    const existingIds = new Set(students.map((student) => clean(student?.id)));
    for (const student of manualStudents) {
      if (!existingIds.has(clean(student?.id))) students.push(student);
    }
  }
  if (excludedStudentIds.length) {
    const excluded = new Set(excludedStudentIds);
    students = students.filter((student) => !excluded.has(clean(student?.id)));
  }
  return students.map((student) => {
    ...
    return {
      ...
      fatherName: clean(student?.shmHb),
      motherName: clean(student?.shmHm),
...
export async function updateAttendanceSessionManualStudents(sessionId, manualStudentIds = []) {
  await initDb();
  ...
  const normalizedManualStudentIds = unique(
    (Array.isArray(manualStudentIds) ? manualStudentIds : [manualStudentIds])
      .map(clean)
      .filter(Boolean)
  );

  const hasFilters = Boolean(session.institutionFilter?.length || session.classFilter?.length || session.registrationFilter?.length || session.familyStatusFilter?.length || (session.institution && session.institution !== "ALL"));
  const nextRosterMode = session.rosterMode === "manual"
    ? "manual"
    : (hasFilters ? "filters_editable" : "manual");
  await sql`
    UPDATE attendance_sessions
    SET manual_student_ids = ${normalizedManualStudentIds},
        roster_mode = ${nextRosterMode},
        excluded_student_ids = ARRAY[]::TEXT[],
        updated_at = NOW()
    WHERE id = ${normalizedSessionId}
  `;

  return getAttendanceRoster(normalizedSessionId);
}

export async function setAttendanceSessionManualStudent(sessionId, studentId, selected) {
  await initDb();
  const normalizedSessionId = clean(sessionId);
  const normalizedStudentId = clean(studentId);
  if (!normalizedSessionId || !normalizedStudentId) throw new Error("Missing attendance student selection.");

  const session = await getAttendanceSessionById(normalizedSessionId);
  if (!session) throw new Error("Attendance session not found.");
  const hasFilters = Boolean(session.institutionFilter?.length || session.classFilter?.length || session.registrationFilter?.length || session.familyStatusFilter?.length || (session.institution && session.institution !== "ALL"));
  const nextMode = selected && hasFilters && session.rosterMode === "filters" ? "filters_editable" : (session.rosterMode || (hasFilters ? "filters_editable" : "manual"));
  await sql`
    UPDATE attendance_sessions
    SET manual_student_ids = CASE
      WHEN ${Boolean(selected)} THEN ARRAY(
        SELECT DISTINCT value
        FROM unnest(COALESCE(manual_student_ids, ARRAY[]::TEXT[]) || ARRAY[${normalizedStudentId}]::TEXT[]) AS value
        WHERE value <> ''
        ORDER BY value
      )
      ELSE array_remove(COALESCE(manual_student_ids, ARRAY[]::TEXT[]), ${normalizedStudentId})
    END,
    excluded_student_ids = CASE
      WHEN ${Boolean(selected)} THEN array_remove(COALESCE(excluded_student_ids, ARRAY[]::TEXT[]), ${normalizedStudentId})
      WHEN ${hasFilters} THEN ARRAY(
        SELECT DISTINCT value
        FROM unnest(COALESCE(excluded_student_ids, ARRAY[]::TEXT[]) || ARRAY[${normalizedStudentId}]::TEXT[]) AS value
        WHERE value <> ''
        ORDER BY value
      )
      ELSE COALESCE(excluded_student_ids, ARRAY[]::TEXT[])
    END,
    roster_mode = ${nextMode},
    updated_at = NOW()
    WHERE id = ${normalizedSessionId}
  `;
...
export async function getAttendanceRoster(sessionId) {
  ...
  const [students, records] = await Promise.all([
    listStudentsForAttendanceInstitution({
      ...
      manualStudentIds: session.manualStudentIds,
      rosterMode: session.rosterMode,
      excludedStudentIds: session.excludedStudentIds
    }),
...
export async function updateAttendanceSessionInvitation(sessionId, {
  title = "",
  invitationMessage = "",
  emailSubject = "",
  emailBody = "",
  ...
  whatsappTemplateLanguage = "he",
  whatsappRecipientRoles = ["student"],
  attachment = null,
  ...
  const nextTitle = clean(title) || clean(current.title) || clean(current.displayTitle);
  const nextInvitationMessage = clean(invitationMessage) || clean(current.invitationMessage || current.invitationEmailBody || current.sourceNote);
  const nextEmailSubject = clean(title) || clean(emailSubject) || clean(current.invitationEmailSubject) || nextTitle;
  const nextEmailBody = clean(invitationMessage) || clean(emailBody) || nextInvitationMessage;
  const roles = unique((Array.isArray(whatsappRecipientRoles) ? whatsappRecipientRoles : [whatsappRecipientRoles]).map(clean).filter((value) => ["student", "father", "mother"].includes(value)));
  const attachmentValues = attachment && clean(attachment.objectKey)
    ? {
        objectKey: clean(attachment.objectKey),
  ...
  if (removeAttachment) {
    await sql`
      UPDATE attendance_sessions
      SET title = ${nextTitle},
          invitation_email_subject = ${nextEmailSubject},
          invitation_email_body = ${nextEmailBody},
          invitation_message = ${nextInvitationMessage},
          source_note = ${nextInvitationMessage},
          invitation_email_recipient_roles = ${normalizeAttendanceEmailRecipientRoles(emailRecipientRoles)},
          invitation_whatsapp_template_name = ${clean(whatsappTemplateName)},
          invitation_whatsapp_template_language = ${clean(whatsappTemplateLanguage) || "he"},
          invitation_whatsapp_recipient_roles = ${roles},
          invitation_attachment_object_key = NULL,
          invitation_attachment_file_name = NULL,
          invitation_attachment_content_type = NULL,
          invitation_attachment_size_bytes = NULL,
          updated_at = NOW()
      WHERE id = ${normalizedSessionId}
    `;
  } else if (attachmentValues) {
    await sql`
      UPDATE attendance_sessions
      SET title = ${nextTitle},
          invitation_email_subject = ${nextEmailSubject},
          ...
          invitation_whatsapp_template_language = ${clean(whatsappTemplateLanguage) || "he"},
          invitation_whatsapp_recipient_roles = ${roles},
          invitation_attachment_object_key = ${attachmentValues.objectKey},
          invitation_attachment_file_name = ${attachmentValues.fileName},
          invitation_attachment_content_type = ${attachmentValues.contentType},
          invitation_attachment_size_bytes = ${attachmentValues.sizeBytes},
          updated_at = NOW()
      WHERE id = ${normalizedSessionId}
    `;
  } else {
...
export async function updateAttendanceSessionDetails(sessionId, {
  ...
  const nextCommunicationAudience = communicationAudience === undefined
    ? current.communicationAudience
    : normalizeCommunicationAudience(communicationAudience);
  const nextResponsibleUserIds = responsibleUserIds === undefined && responsibleUserId === undefined
    ? current.responsibleUserIds || []
    : unique([
      ...(Array.isArray(responsibleUserIds) ? responsibleUserIds : [responsibleUserIds]),
      responsibleUserId
    ].map(clean).filter(Boolean));
  const nextResponsibleUserId = nextResponsibleUserIds[0] || null;
  ...
  await sql`
    ...
    SET
      ...
      invitation_email_subject = ${nextTitle || ATTENDANCE_SESSION_TYPE_LABELS[nextSessionType]},
      invitation_email_body = ${nextSourceNote},
      invitation_message = ${nextSourceNote},
      communication_audience = ${nextCommunicationAudience},
      session_type = ${nextSessionType},
      session_date = ${nextSessionDate},
      responsible_user_id = ${nextResponsibleUserId},
      responsible_user_ids = ${nextResponsibleUserIds},
