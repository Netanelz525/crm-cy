...
function normalizeUpdatesLockedUntil(value) {
  const raw = clean(value);
  if (!raw) return null;
  const parsed = new Date(raw);
...
function isSessionUpdatesLocked(session) {
  const lockedUntil = clean(session?.updatesLockedUntil);
  if (!lockedUntil) return false;
  const cutoff = new Date(lockedUntil);
...
function mapSessionRow(row) {
  ...
  return {
    ...
    updatesLockedUntil: row.updates_locked_until ? new Date(row.updates_locked_until).toISOString() : null,
    isUpdatesLocked: isSessionUpdatesLocked({ updatesLockedUntil: row.updates_locked_until ? new Date(row.updates_locked_until).toISOString() : null }),
...
export async function createAttendanceSession({
  ...
  const normalizedUpdatesLockedUntil = normalizeUpdatesLockedUntil(updatesLockedUntil);
...
export async function updateAttendanceSessionDetails(sessionId, {
  ...
  const nextUpdatesLockedUntil = updatesLockedUntil === undefined
    ? current.updatesLockedUntil
    : normalizeUpdatesLockedUntil(updatesLockedUntil);
  ...
  await sql`
    ...
    SET
      ...
      updates_locked_until = ${nextUpdatesLockedUntil},
...
export async function saveAttendanceRecord({ sessionId, record, markedByUserId }) {
  ...
  const current = await getAttendanceSessionById(normalizedSessionId);
  if (!current) throw new Error("Attendance session not found.");
  if (isSessionUpdatesLocked(current)) throw new Error("המפגש נעול לעדכוני נוכחות. יש לפנות למנהל לפתיחה מחדש.");
