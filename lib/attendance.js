...
function normalizeAttendanceStatusFilters(values) {
  return unique((Array.isArray(values) ? values : [values])
    .map((value) => normalizeAttendanceStatus(value))
    .filter((value) => ATTENDANCE_STATUS_LABELS[value]));
}
...
function mapSessionRow(row) {
  ...
  return {
    ...
    emailResponseStatuses: normalizeAttendanceStatusFilters(row.email_response_statuses),
    emailResponseStatusOptions: mapFilterOptions(normalizeAttendanceStatusFilters(row.email_response_statuses), ATTENDANCE_STATUS_LABELS),
    emailRecipientRoles: normalizeAttendanceEmailRecipientRoles(row.email_recipient_roles),
    emailRecipientRoleOptions: mapFilterOptions(normalizeAttendanceEmailRecipientRoles(row.email_recipient_roles), ATTENDANCE_EMAIL_RECIPIENT_LABELS),
...
export async function createAttendanceSession({
  ...
  const normalizedEmailResponseStatuses = normalizeAttendanceStatusFilters(emailResponseStatuses);
...
export async function updateAttendanceSessionMessaging(sessionId, {
  ...
  await sql`
    ...
    SET
      ...
      email_response_statuses = ${normalizeAttendanceStatusFilters(emailResponseStatuses)},
