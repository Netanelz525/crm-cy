...
export async function listOpenAttendanceSessionsForStudent(studentId, { limit = 8, scanLimit = 30 } = {}) {
  await initDb();
  const normalizedStudentId = clean(studentId);
  if (!normalizedStudentId) return [];
  const safeLimit = Math.max(1, Math.min(20, Number(limit) || 8));
  const safeScanLimit = Math.max(safeLimit, Math.min(60, Number(scanLimit) || 30));
