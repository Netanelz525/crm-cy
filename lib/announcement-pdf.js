...
function bodyStyle(layout = {}) {
  const body = layout?.body || {};
  return [
    `top:${clampPercent(body.top, 27)}%`,
    `bottom:${clampPercent(body.bottom, 18)}%`,
    `right:${clampPercent(body.right, 10)}%`,
    `left:${clampPercent(body.left, 10)}%`,
    `font-size:${clampNumber(body.fontSize, 24, 12, 72)}px`,
    `line-height:${clampNumber(body.lineHeight, 1.55, 1, 2.4)}`,
    `text-align:${["right", "center", "left"].includes(clean(body.textAlign)) ? clean(body.textAlign) : "center"}`
  ].join(";");
}
...
function buildHtml({ announcement, template, backgroundUrl, fontUrl }) {
  ...
  const style = bodyStyle(announcement?.layoutOverride);
