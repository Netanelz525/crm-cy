...
const HEBREW_MONTH_NUMBER_TO_CODE = Object.fromEntries(
  HEBREW_MONTH_OPTIONS.map((option) => [option.monthNumber, option.value])
);
...
function getMonthCodeByNumber(monthNumber) {
  return HEBREW_MONTH_NUMBER_TO_CODE[Number(monthNumber)] || "";
}

function resolveActualMonthOption(monthNumber, year) {
  const monthCode = getMonthCodeByNumber(monthNumber);
  if (!monthCode) return null;

  if (!HDate.isLeapYear(year) && monthCode === "ADAR_II") {
    return getMonthOption("ADAR_I");
  }

  return getMonthOption(monthCode);
}

function getOccurrenceForYear(day, monthOption, year) {
  const isLeapYear = HDate.isLeapYear(year);
  const requestedMonthNumber = !isLeapYear && monthOption.value === "ADAR_II"
    ? months.ADAR_I
    : monthOption.monthNumber;
  const requestedDaysInMonth = HDate.daysInMonth(requestedMonthNumber, year);
  const occurrenceDate = new HDate(day, requestedMonthNumber, year);
  const actualDay = Number(occurrenceDate.getDate());
  const actualMonthNumber = Number(occurrenceDate.getMonth());
  const actualMonthOption = resolveActualMonthOption(actualMonthNumber, year);
  if (!actualMonthOption) return null;

  const dayRolledForward = day > requestedDaysInMonth;
  const adarMerged = !isLeapYear && monthOption.value === "ADAR_II";
  const actualHebrewDateLabel = `${actualDay} ${actualMonthOption.label}`;

  let adjustmentNote = "";
  if (adarMerged && dayRolledForward) {
    adjustmentNote = `בשנה רגילה אדר ב׳ מתאחד עם אדר, וכיוון שאין ל׳ בחודש האירוע נדחה ל־${actualHebrewDateLabel}.`;
  } else if (adarMerged) {
    adjustmentNote = "בשנה רגילה אירועי אדר א׳ ואדר ב׳ מאוחדים לחודש אדר.";
  } else if (dayRolledForward) {
    adjustmentNote = `השנה אין ל׳ בחודש הזה, ולכן האירוע נדחה ל־${actualHebrewDateLabel}.`;
  }

  return {
    hdate: occurrenceDate,
    year,
    actualHebrewDateLabel,
    actualHebrewDay: actualDay,
    actualHebrewMonthCode: actualMonthOption.value,
    dayRolledForward,
    adarMerged,
    adjustmentNote
  };
...
function buildOccurrenceResult(occurrence, todayUtc) {
  if (!occurrence?.hdate) return null;
  const gregDate = occurrence.hdate.greg();
  const dateInput = formatJerusalemDateInput(gregDate);
  const candidateUtc = dateOnlyToUtc(dateInput);

  return {
    hebrewYear: occurrence.year,
    hebrewDateDisplay: occurrence.actualHebrewDateLabel,
    hebrewDay: occurrence.actualHebrewDay,
    hebrewMonthCode: occurrence.actualHebrewMonthCode,
    ...
    wasAdjusted: occurrence.dayRolledForward || occurrence.adarMerged,
    dayRolledForward: occurrence.dayRolledForward,
    adarMerged: occurrence.adarMerged,
...
export function getNextStudentEventOccurrence(hebrewDay, hebrewMonthCode, fromDate = new Date()) {
  return getStudentEventOccurrenceWindow(hebrewDay, hebrewMonthCode, fromDate)?.next || null;
}

export function getStudentEventOccurrenceWindow(hebrewDay, hebrewMonthCode, fromDate = new Date()) {
  const day = normalizeHebrewDay(hebrewDay);
  const monthOption = getMonthOption(hebrewMonthCode);
  if (!day || !monthOption) return { previous: null, next: null };
  ...
  const candidateYears = Array.from({ length: 8 }, (_, index) => currentHebrewYear - 3 + index);
  const occurrences = [];

  for (const year of candidateYears) {
    const occurrence = getOccurrenceForYear(day, monthOption, year);
    if (!occurrence?.hdate) continue;
    const result = buildOccurrenceResult(occurrence, todayUtc);
    if (result) occurrences.push(result);
  }

  occurrences.sort((left, right) => dateOnlyToUtc(left.gregorianDate) - dateOnlyToUtc(right.gregorianDate));

  let previous = null;
  let next = null;

  for (const occurrence of occurrences) {
    if (Number(occurrence.daysUntil) < 0) {
      previous = occurrence;
      continue;
    }
    next = occurrence;
...
export async function updateStudentEvent({
  ...
  if (!normalizedId) throw new Error("לא נבחר אירוע.");
  if (!normalizedStudentId) throw new Error("לא נבחר תלמיד.");
  if (!normalizedEventType) throw new Error("יש לבחור סוג אירוע.");
  if (normalizedEventType === "other" && !normalizedCustomLabel) throw new Error("יש להזין סוג אירוע חופשי.");
  if (!normalizedHebrewDay) throw new Error("יש לבחור יום עברי תקין.");
  ...
  const rows = await sql`
    ...
    SET
      ...
      custom_event_label = ${normalizedEventType === "other" ? normalizedCustomLabel : null},
      note_text = ${normalizedNoteText || null},
      hebrew_day = ${normalizedHebrewDay},
      hebrew_month_code = ${normalizedHebrewMonthCode},
      updated_at = NOW()
  ...
  if (!rows[0]) throw new Error("האירוע לא נמצא בכרטיס התלמיד.");
  return mapStudentEventRow(rows[0]);
}

export async function deleteStudentEvent({ id, studentId }) {
  await initDb();
  const normalizedId = clean(id);
  const normalizedStudentId = clean(studentId);
  if (!normalizedId) throw new Error("לא נבחר אירוע.");
  ...
  if (!rows[0]) throw new Error("האירוע לא נמצא או שכבר נמחק.");
...
export async function listStudentEventReminderDigest({ daysAhead = 10, daysBack = 10, fromDate = new Date() } = {}) {
  await initDb();
  const rows = await sql`
    SELECT
      e.id,
  ...
  const normalizedAhead = Math.max(1, Number(daysAhead) || 10);
  const normalizedBack = Math.max(1, Number(daysBack) || 10);
  const baseEvents = rows.map(mapStudentEventRow);
  const upcoming = [];
  const recentPast = [];

  for (const event of baseEvents) {
    const window = getStudentEventOccurrenceWindow(event.hebrewDay, event.hebrewMonthCode, fromDate);

    if (window?.next && Number(window.next.daysUntil) >= 0 && Number(window.next.daysUntil) <= normalizedAhead) {
      upcoming.push({
        ...event,
        relevantOccurrence: window.next
      });
    }

    if (window?.previous && Number(window.previous.daysUntil) < 0 && Math.abs(Number(window.previous.daysUntil)) <= normalizedBack) {
      recentPast.push({
        ...event,
        relevantOccurrence: window.previous
      });
    }
  }

  const upcomingSorter = (left, right) =>
    Number(left?.relevantOccurrence?.daysUntil || 0) - Number(right?.relevantOccurrence?.daysUntil || 0)
    || String(left?.studentName || "").localeCompare(String(right?.studentName || ""), "he")
    || String(left?.eventLabel || "").localeCompare(String(right?.eventLabel || ""), "he");

  const recentPastSorter = (left, right) =>
    Number(right?.relevantOccurrence?.daysUntil || 0) - Number(left?.relevantOccurrence?.daysUntil || 0)
    || String(left?.studentName || "").localeCompare(String(right?.studentName || ""), "he")
    || String(left?.eventLabel || "").localeCompare(String(right?.eventLabel || ""), "he");

  upcoming.sort(upcomingSorter);
  recentPast.sort(recentPastSorter);

  return {
    upcoming,
    recentPast,
    daysAhead: normalizedAhead,
    daysBack: normalizedBack
  };
}
