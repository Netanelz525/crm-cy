...
function getMessagesEndpoint() {
  // The operational bot is deliberately isolated from the Dualhook
  // coexistence/broadcast connection. It always uses the direct Meta
  // credentials for the operational WhatsApp number.
  return `https://graph.facebook.com/${getGraphApiVersion()}/${getPhoneNumberId()}/messages`;
}

function getOperationalTemplateOwnerId() {
  return clean(process.env.WHATSAPP_WABA_ID)
    || getPhoneNumberId();
}
...
export function isWhatsAppTemplateConfigured() {
  return Boolean(clean(process.env.WHATSAPP_ACCESS_TOKEN));
}
...
export async function listWhatsAppApprovedTemplates() {
  ...
  const response = await fetch(`https://graph.facebook.com/${getGraphApiVersion()}/${getOperationalTemplateOwnerId()}/message_templates`, {
