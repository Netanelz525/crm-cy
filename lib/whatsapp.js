...
import { coexistenceWhatsAppConfig, isCoexistenceWhatsAppConfigured, operationalWhatsAppConfig } from "./whatsapp-channel-config.mjs";
...
export function getWhatsAppWebhookAppSecret() {
  return clean(process.env.WHATSAPP_APP_SECRET || process.env.META_APP_SECRET);
}
...
function getMessagesEndpoint() {
  return operationalWhatsAppConfig().messagesUrl;
}

export function isWhatsAppTemplateConfigured() {
  return isCoexistenceWhatsAppConfigured();
}
...
function normalizeTemplate(row) {
  ...
  const bodyExample = Array.isArray(body?.example?.body_text?.[0]) ? body.example.body_text[0].map(clean) : [];
  return {
    name: clean(row?.name),
    ...
    bodyText,
    parameterCount,
    bodyVariables: Array.from({ length: parameterCount }, (_, offset) => ({
      index: offset + 1,
      example: bodyExample[offset] || ""
    })),
    ...
    buttons: (Array.isArray(buttonsComponent?.buttons) ? buttonsComponent.buttons : []).map((button) => ({
...
export async function listWhatsAppApprovedTemplates() {
  ...
  const response = await fetch(`${config.templatesUrl}?${templateQuery.toString()}`, {
