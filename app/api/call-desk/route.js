...
export async function POST(request) {
  ...
  try {
    ...
    if (body.kind === "assign") {
      ...
      return NextResponse.json({ ok: true, assignment: await assignStudentCall({ studentId: body.studentId, assigneeUserId: body.assigneeUserId, assigneeStudentId: body.assigneeStudentId, assignedByUserId: user.clerk_user_id }) });
    }
    ...
    if (body.completed) await completeStudentCall({ studentId: body.studentId, userId: user.clerk_user_id, linkedStudentId: user.linked_student_id });
