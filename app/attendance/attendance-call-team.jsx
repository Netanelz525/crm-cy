...
export default function AttendanceCallTeam({sessionId,team,rosterIds}) {
  ...
  async function manage(body){setBusy(true);setMessage("");try{const response=await fetch(`/api/attendance/sessions/${encodeURIComponent(sessionId)}/calls`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const result=await response.json();if(!response.ok)throw new Error(result.error);setMessage(body.kind==="requeue"?"התלמיד הוחזר לתור.":"תיעוד השיחה עודכן.");router.refresh();}catch(error){setMessage(error.message||"העדכון נכשל.");}finally{setBusy(false);}}
  function updateAttempt(event,a){event.preventDefault();const form=new FormData(event.currentTarget);manage({kind:"manage_attempt",token:a.token,outcome:form.get("outcome"),attendanceStatus:form.get("attendanceStatus"),note:form.get("note")});}
  return <details className="card"><summary><strong>צוות מתקשרים למפגש</strong> · טופלו {completed} מתוך {rosterIds.length}</summary>
    ...
    <details><summary>תיעוד השיחות האחרונות</summary><div className="call-attempt-list">{team.attempts.map(a=>{const student=team.students.find(s=>s.id===a.student_id);return <form className="call-attempt-edit" key={a.token} onSubmit={e=>updateAttempt(e,a)}><div className="call-attempt-title"><b>{student?.label||a.student_id}</b><span>{new Date(a.created_at).toLocaleString("he-IL")} · {a.caller_name||"—"}</span></div><label>תוצאת שיחה<select name="outcome" defaultValue={a.outcome}>{Object.entries(outcomes).filter(([v])=>v!=="pending").map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>סטטוס הגעה בפועל<select name="attendanceStatus" defaultValue={student?.status||a.attendance_status||""}><option value="">ללא סטטוס</option>{team.statusOptions.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>הערה<input name="note" defaultValue={a.note}/></label><div className="quick-actions"><button disabled={busy}>שמור עדכון</button><button type="button" disabled={busy} onClick={()=>manage({kind:"requeue",studentId:a.student_id})}>החזר לתור</button></div></form>})}</div></details>
  </details>;
}
